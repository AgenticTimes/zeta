...
- **正向队列（Agent-1）**：roadmap2.md §2 主链的「下一批候选」，按实测损害量序。当前（批次 421 后）：
  **头名＝`[dynamic]` 幽灵 `copy`×4 的定价批**（`jq_wufu_local.py:166/190/248/301`，全是 `market_df[mask].copy()`；`dropna`×15 实测在 `jq_wufu_local.py` 内 0，故对当前主线语料暂不构成拦路）—— 定价内容＝这 4 个点在晨间崩点（`str_trim`）**之前还是之后**（421 已证：`extract_result` 那 22 个点执行序未证 ⇒ 同一判据要重走），若在执行序内则真修要 Series + `DataFrame.index` 的 `copy` 语义，属**结构面**（用户裁定：别再一个一个补语法缺口）
  → **第 2 格＝`str_trim + 24` 读 `x0=0x3532`**（419 的崩点，420/421 两批 A/B 复测仍在 135 行、**位移 0 有正证据**：改前二进制仍死在 `fetch_stocks`。生产者三条负结果入册，真修大概率要动 `runtime/py_additions.c` ⇒ **等用户授权**，未获授权前不作为头名）
