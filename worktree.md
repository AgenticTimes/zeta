...
- **正向队列（Agent-1）**：roadmap2.md §2 主链的「下一批候选」，按实测损害量序。当前（批次 423 后）：
  **头名＝`jq_wufu.py:702` 的 `for _name, code in indexes.items()` 收到的接收者是**当天日期**** —— 423 的 38 条点名下来的头两条之一：`indexes` 是 :695-699 的字典字面量，同函数 :710 另有 `today = context.current_dt.date()` ⇒ 强烈指向**槽位复用/串号**（#130/#131/#142 同族），不是缺语法。IR 判据现成：`py_map_items` 语料 25 个调用点／23 个宿主、`check_a_share_weak_period` 内恰好 1 个。第 2 条点名（`map_get` 收浮点文本串）的前置是**让点名句带上用户函数名**（`dladdr` 旁边再挂一层返回地址符号化，`backtrace_symbols` 291 用过）
  ...
  → **第 3 格＝`str_trim + 24` 读 `x0=0x3532`**（419 的崩点；423 之后这条已不是当前死点 —— 守卫版跑完不再崩，但它仍在 #145 的账上：packed str 生产者未定位）。生产者三条负结果入册，真修大概率要动 `runtime/py_additions.c` ⇒ **等用户授权**，未获授权前不作为头名
  → **第 4 格＝#134 `GroupBy.__len__` 恒 0 / #117 动态键 `.get(k, default)` 语料 7 形仍落 `_get`**（都不等授权）
