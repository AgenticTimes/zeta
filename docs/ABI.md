...
在能用与不能用之间翻转"，:782 的 `all_mirs.sort_by(...)` 就是这把锁的钥匙。
...
9. ✅ **编译期诊断在门禁里读不出来**（批次 319 撞出 → **批次 320 关闭**）：
   ...
   **口径**：只统计 `warning:` / `PY-A:`，排除 `clang: warning:`（链接器抱怨 `-no-pie`，
   ...
   - **批次 322 顺带修掉本项自己的一个取证缺陷**（`tools/run_all.sh:188`）：写进
...
11. **`official` 那一个数把"编译器接不接受这段源码"和"程序能否链上完整运行时"混成了同一件事**
   ...
   **后果**：只要判据仍是"编译+链接全过"，解析恢复就被运行时完整度**封顶**——每恢复一行只要引用
   ...
   - `tools/run_all.sh:93` 只在"整链失败"时补跑一次 `--no-link` 做归因，
   ...
   - **判据（`tools/run_all.sh:430`）从 `pass==total` 改为 `compile==total`**，
