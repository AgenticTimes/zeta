...
一个格子只装一个 64 位词）。**函数体里没有这个名的槽时，读走 env**（gen.rs:3795 起），
所以只写局部槽的写路径等于把这次赋值丢掉。三条写路径现在是这样：
① `=` —— 写局部槽 + 镜像 env（gen.rs:3795 起，本规则的第一处实现）；
...
**三条路径镜像的都是"刚写过的那个槽"，不是原来那条表达式**（`=`：gen.rs:3795 传
`existing`、gen.rs:3795 传 `new_id`；`+=`：gen.rs:3795 传 `slot_id`）。理由实测：折叠出的加法表达式里就带着这个槽
...
模块全局用的是那条 `=` 留下的局部槽 —— 读侧先看这个函数已有的槽（gen.rs:3452，在 gen.rs:3795
...
（gen.rs:10498-10508 注释、:14079-14084 "The closure value is then the function address
(i64)"）。
...
分派守卫 gen.rs:10965-10989：`receiver.is_none()` 且 `arg_ids.len() <= 4` 且名字不是
...
gen.rs `fill` 按**声明顺序**落槽：位置实参（:8845-8851）→ 关键字（:8852-8857）→
`**` 映射填未绑定槽（:8858-8870）→ 默认值（:8871-8880）。
C 侧 `zeta_param_default` 是**恒等 no-op**（py_additions.c:2672），存在只为让标记不成未定义符号。
> 锚点源码：src/middle/mir/gen.rs
现状记录（不是愿望）：**重复绑定静默后者覆盖**（:8854 `slots[i] = Some(v)` 无检查）、
**未知关键字变成多余位置实参**（:8855 `slots.push`）→ 再由 §3.2#10 静默裁掉；
...
静态展开（gen.rs:9009-9045，"V1: static-size arrays compile-time unrolled"）；
日志调用里的 starred 实参明确**不展开**并告警（gen.rs:6442-6463）。
...
MIR 打类型 `Type::Named("PyArgNS")`（gen.rs:6065-6077）、消费端按字段访问分派到
`py_argparse_get_{i64,f64,bool,str}`（gen.rs:12074-12107）。
...
锚点 resolver.rs:2008、:2809；mir/gen.rs:392、:635、:747、:754、:3764。
...
**N3 重载消歧形是 `<name>_<实参数>`，由 MIR 生成侧加**：gen.rs:11254、:12787
（`format!("{}_{}", func, arg_ids.len())`）。同处注释分三段：
加后缀的理由（src/middle/mir/gen.rs:11228-11230）、三类**不加**后缀的名字
（`zeta_*`、含 `__`、含 `::`，src/middle/mir/gen.rs:11231-11241）、以及"读侧剥后缀会误伤
名字自带的下划线"的现场记录（src/middle/mir/gen.rs:11242-11248：`DataFrame::reset_index`
...
（gen.rs:9549 `func: "[dynamic]str__map"`）；C 侧的实现叫 `zt_dyn_str_map`，
...
且已经有只造了名、没注册实现的变体（gen.rs:9445、:9464、:9483、:9502、:9562、:13461 注释里的
...
| ② gen.rs 分发 | 手工 | 例 gen.rs:9549、:9801 |
...
| `zeta_dyn_getitem` | :3427 | `base` 是 **L2 数据指针**或 **L3 map 句柄**；`key` 是索引，**负数按 `+len` 回卷**（:3433）；其 vec 判据写作 `cap >= 8`（:3432）⚠️ **与 L2 的 `cap >= 1`（:3474）不一致，且本批实测这是一个可观测的死循环**（`[1,2]+[3,4,5]` 的 cap=5 走不进 vec 臂 ⇒ 落到 `map_get` 的开放寻址环，`&(cap-1)` 在 cap=5 上不是掩码 ⇒ 永不停止；详见附 B#7） | gen.rs:13433；**手写**声明 codegen.rs:1079（2 参） |
| `zeta_dyn_len` | :3492 | 句柄**或**文本指针，**无标签**；读序 map→vec→文本（R4） | gen.rs:7137 |
| `zeta_dyn_contains` | :3510 | 4 元：容器 + 原键 + **map 归一键**（`map_str_key` 之值）+ `key_is_str` 选内容相等（:3506 原文） | gen.rs:9801 |
| `zeta_dyn_truth` | :3534 | map→已用槽数、vec→头长度、文本→首字节（:3529 原文）；文本分支先过 L7 的地址闸门（:3539） | gen.rs:4075 |
...
10. **`official: 194/194` 里的用例程序被解析器就地截断**（批次 320 收门禁 stderr 时
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **修法归属**：G.1/G.3（解析器 + 用例重构），不在 ABI 范围；优先级按上表丢行量。
     ...
     （`src/middle/mir/gen.rs:8734`），而 JIT 表里只有 `println_i64` ⇒ 新解析出的代码
   ...
   - **批次 323 关掉第三族：`s[i..]` 开区段下标**（任务 #39）。两层：
     ...
     **为何不在 `parse_expr` 里修**：优先级链是
     ...
     `src/middle/mir/gen.rs:12686`），`String::new()` 下成空串字面量（`:12690`；
