...
| 2 | `f64` | **IEEE-754 位模式**，不是裸整数 | ① 静态 `double` 槽：`Assign` 时 int 值走 `slot_sitofp`（codegen.rs:3761-3786）；② struct 字段是 64 位槽：存前 bitcast（codegen.rs:7144） | 从**裸容器字**读回必须是 `bitcast`，不能 `sitofp`（`slot_or_sitofp` codegen.rs:1941-1956，判据 `slot_read_ids` 60 / 1593） | 批次 298：裸存 → 读回 4.94e-322；批次 299：缺 `sitofp` → `500.0 + 50` 算成 `4.6e18`（5782 记录） |
...
| 4 | `str`（指针形） | `.rodata` 或 GC 块首地址，`ptrtoint` 成 i64 | `MirExpr::StringLit`：私有 global 字节数组 + `ptrtoint`（codegen.rs:6634-6653） | 可直接解引用；`GC_base(h) != 0` ⇒ 堆串 | 与 #5 不可静态区分 ⇒ 见 R3 |
...
slot is int`（锚点 codegen.rs:3795 调用点、codegen.rs:7808 实现），并在 `report_abi_coercions`
...
:7182-7183）；元组走 `StackArray` 的 `[cap|len|elems…]`、句柄 = `buf+16`（:6740-6745、
:7583-7584，与 §1#6 同一几何）。读侧：字段访问 = `int_to_ptr` + **整块 load**
`struct_type(&[i64; N], false)`（:6501-6509，非 packed）+ `extract_value`（:7313-7316）；
字段写 = 地址算术 `base + idx*8` 后 `store`（:5909-5930）。
⚠️ **另一套通用映射器不参与签名**：`type_to_llvm_type`（:8191）会把 `Type::Tuple`
映成真 LLVM struct（:8297-8303）。改签名时只认 :1467-1480，认它就等着 ABI 不一致。
...
|---|---|---|---|---|---|
| 1 | `i<n>` → `i<m>`，n<m | `arg_zext`（:7692-7699） | 否 | 无 | **保留** |
| 2 | `i<n>` → `i<m>`，n>m | `arg_trunc`（:7709-7712） | **是**（丢高位） | `narrow iA→iB`（:7700-7708） | **删除候选**（改诊断） |
| 3 | 同宽整数 | 原样（:7713-7715） | 否 | 无 | 保留 |
| 4 | int → float，且该实参来自容器裸槽 | `arg_slot_bits` **bitcast**（:7717-7728） | 否 | 无 | **保留**（与 §2 R2 对称，批次 299；判据 `slot_read_ids`） |
| 5 | int → float，其他来源 | `arg_sitofp`（:7729-7732） | 否 | 无 | 保留，但**应告警化**（值对、静默错） |
| 6 | float → int | `arg_fptosi`（:7744-7747） | **是**（小数；负数/OOB 为 LLVM poison） | `fptosi → iN`（:7738-7743） | **删除候选** |
| 7 | float ↔ ptr | **不转换**，原值照推（:7749-7753） | 值不匹配照样进 `build_call` | `float↔ptr (forbidden)` | **必改**：写着 forbidden 却不拒绝 |
| 8 | 其余一切（**含 ptr ↔ i64**） | 原样（:7754 `_ => push`） | 未知 | 无 | **删除候选**（今天的默认＝硬塞） |
| 9 | 实参少于形参 | 补 null/零常量（:7757-7778） | 静默补空 | 无 | **删除候选**（Python 侧应报错） |
| 10 | 实参多于形参 | `result.truncate(n_params)`（:7779-7780） | **静默丢实参** | 无 | **删除候选** |
...
（gen.rs:10498-10508 注释、:14577-14582 "The closure value is then the function address
(i64)"）。
...
分派守卫 gen.rs:11415-11439：`receiver.is_none()` 且 `arg_ids.len() <= 4` 且名字不是
...
gen.rs `fill` 按**声明顺序**落槽：位置实参（:9218-9224）→ 关键字（:9225-9230）→
`**` 映射填未绑定槽（:9231-9243）→ 默认值（:9244-9253）。
C 侧 `zeta_param_default` 是**恒等 no-op**（py_additions.c:2672），存在只为让标记不成未定义符号。
> 锚点源码：src/middle/mir/gen.rs
现状记录（不是愿望）：**重复绑定静默后者覆盖**（:9227 `slots[i] = Some(v)` 无检查）、
**未知关键字变成多余位置实参**（:9228 `slots.push`）→ 再由 §3.2#10 静默裁掉；
...
静态展开（gen.rs:9383-9419，"V1: static-size arrays compile-time unrolled"）；
日志调用里的 starred 实参明确**不展开**并告警（gen.rs:6815-6836）。
...
MIR 打类型 `Type::Named("PyArgNS")`（gen.rs:6438-6450）、消费端按字段访问分派到
`py_argparse_get_{i64,f64,bool,str}`（gen.rs:12544-12577）。
...
锚点 resolver.rs:2011、:2822；mir/gen.rs:418、:661、:773、:780、:4125。
...
**N3 重载消歧形是 `<name>_<实参数>`，由 MIR 生成侧加**：gen.rs:11721、:13285
（`format!("{}_{}", func, arg_ids.len())`）。同处注释分三段：
加后缀的理由（src/middle/mir/gen.rs:11694-11696）、三类**不加**后缀的名字
（`zeta_*`、含 `__`、含 `::`，src/middle/mir/gen.rs:11697-11707）、以及"读侧剥后缀会误伤
名字自带的下划线"的现场记录（src/middle/mir/gen.rs:11708-11714：`DataFrame::reset_index`
...
⇒ 别名表与**IR 发射顺序**是一对锁死件：src/main.rs:944-947 的注释原文——
HashMap 迭代顺序随机 ⇒ `print.N` 冲突改名和运行期别名表"从一次运行到下一次
在能用与不能用之间翻转"，:948 的 `all_mirs.sort_by(...)` 就是这把锁的钥匙。
...
（gen.rs:9940 `func: "[dynamic]str__map"`）；C 侧的实现叫 `zt_dyn_str_map`，
...
且已经有只造了名、没注册实现的变体（gen.rs:9836、:9855、:9874、:9893、:9953、:13959 注释里的
...
| ② gen.rs 分发 | 手工 | 例 gen.rs:9940、:10192 |
...
| `zeta_dyn_getitem` | :3427 | `base` 是 **L2 数据指针**或 **L3 map 句柄**；`key` 是索引，**负数按 `+len` 回卷**（:3433）；其 vec 判据写作 `cap >= 8`（:3432）⚠️ **与 L2 的 `cap >= 1`（:3474）不一致，且本批实测这是一个可观测的死循环**（`[1,2]+[3,4,5]` 的 cap=5 走不进 vec 臂 ⇒ 落到 `map_get` 的开放寻址环，`&(cap-1)` 在 cap=5 上不是掩码 ⇒ 永不停止；详见附 B#7） | gen.rs:13931；**手写**声明 codegen.rs:1127（2 参） |
| `zeta_dyn_len` | :3492 | 句柄**或**文本指针，**无标签**；读序 map→vec→文本（R4） | gen.rs:7510 |
| `zeta_dyn_contains` | :3510 | 4 元：容器 + 原键 + **map 归一键**（`map_str_key` 之值）+ `key_is_str` 选内容相等（:3506 原文） | gen.rs:10192 |
| `zeta_dyn_truth` | :3534 | map→已用槽数、vec→头长度、文本→首字节（:3529 原文）；文本分支先过 L7 的地址闸门（:3539） | gen.rs:4448 |
...
10. **`official: 194/194` 里的用例程序被解析器就地截断**（批次 320 收门禁 stderr 时
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **修法归属**：G.1/G.3（解析器 + 用例重构），不在 ABI 范围；优先级按上表丢行量。
     ...
     （`src/middle/mir/gen.rs:9107`），而 JIT 表里只有 `println_i64` ⇒ 新解析出的代码
   ...
   - **批次 322 关掉第二族：字符串字面量作 match 模式**（`parse_lit` 只吃数字，
     ...
     **五条 expect 全部落到 `_ =>`**。原因：`==` 在 IR 里被声明为
     ...
     （与 `op == "+"` 同形，后端 `codegen.rs:6822` 有 str 比较分支）。
   ...
   - **批次 323 关掉第三族：`s[i..]` 开区段下标**（任务 #39）。两层：
     ...
     **为何不在 `parse_expr` 里修**：优先级链是
     ...
     会当场崩编译器：`src/backend/codegen/codegen.rs:7143` 无条件索引 `exprs[field_id]`。
     触发链 `tests/unit-tests/minimal_compiler.z:129` → 崩点 `codegen.rs:7143`（rc=101）。
     修法：`Box::new` 走**恒等**下型（`Box<T>` 槽与其内值同为 64 位句柄，
     `src/middle/mir/gen.rs:13184`），`String::new()` 下成空串字面量（`:13188`；
   ...
   - **批次 325 推进第五族：范围模式（`'a'..='z'` / `1..=10` / `x @ 1..=10`）——
     ...
     **从没进过 `exprs`**，而 `gen_expr_safe`（`src/backend/codegen/codegen.rs:6132`）
     ...
     一行输出都没有**。修法是把 guard 的 dest 直接交给上层读（`gen.rs:12283`）。
     ④ `inclusive` 字段此前被 `inclusive: _` 丢掉 ⇒ `..` 一直按 `..=` 运行。
     现已区分（`gen.rs:3857`）。official 194 文件里**没有一处**用排他范围作模式
...
11. **`official` 那一个数把"编译器接不接受这段源码"和"程序能否链上完整运行时"混成了同一件事**
   ...
   **后果**：只要判据仍是"编译+链接全过"，解析恢复就被运行时完整度**封顶**——每恢复一行只要引用
   ...
   - `src/main.rs:650` + `:1003` 新增 `--no-link`（出 `.o` 即止）；
