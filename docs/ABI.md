...
（gen.rs:10498-10508 注释、:14370-14375 "The closure value is then the function address
(i64)"）。
...
`py_argparse_get_{i64,f64,bool,str}`（gen.rs:12337-12370）。
...
加后缀的理由（src/middle/mir/gen.rs:11487-11489）、三类**不加**后缀的名字
（`zeta_*`、含 `__`、含 `::`，src/middle/mir/gen.rs:11490-11500）、以及"读侧剥后缀会误伤
名字自带的下划线"的现场记录（src/middle/mir/gen.rs:11501-11507：`DataFrame::reset_index`
...
且已经有只造了名、没注册实现的变体（gen.rs:9629、:9648、:9667、:9686、:9746、:13752 注释里的
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **批次 325 推进第五族：范围模式（`'a'..='z'` / `1..=10` / `x @ 1..=10`）——
     ...
     **从没进过 `exprs`**，而 `gen_expr_safe`（`src/backend/codegen/codegen.rs:6011`）
     ...
     一行输出都没有**。修法是把 guard 的 dest 直接交给上层读（`gen.rs:12076`）。
