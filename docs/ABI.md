...
前提（codegen.rs:1642-1646）：**静态可定的本地按自身 LLVM 类型开槽**
...
| 2 | `f64` | **IEEE-754 位模式**，不是裸整数 | ① 静态 `double` 槽：`Assign` 时 int 值走 `slot_sitofp`（codegen.rs:3569-3594）；② struct 字段是 64 位槽：存前 bitcast（codegen.rs:6533） | 从**裸容器字**读回必须是 `bitcast`，不能 `sitofp`（`slot_or_sitofp` codegen.rs:1749-1764，判据 `slot_read_ids` 60 / 1593） | 批次 298：裸存 → 读回 4.94e-322；批次 299：缺 `sitofp` → `500.0 + 50` 算成 `4.6e18`（5782 记录） |
| 3 | `bool` | **i64 的 0/1** | 比较结果立刻 `zext i1→i64`（`eq_ext`/`ne_ext`/…，codegen.rs:4053-4061） | 一律 `!= 0` | `Type::Bool → bool_type`（7388）只在 ABI 边界（参数/返回签名）出现；**i1 从不进槽** |
| 4 | `str`（指针形） | `.rodata` 或 GC 块首地址，`ptrtoint` 成 i64 | `MirExpr::StringLit`：私有 global 字节数组 + `ptrtoint`（codegen.rs:6027-6046） | 可直接解引用；`GC_base(h) != 0` ⇒ 堆串 | 与 #5 不可静态区分 ⇒ 见 R3 |
...
锚点 codegen.rs:3569-3594（`MirStmt::Assign` 的 `slot_sitofp`）。
...
锚点 `slot_or_sitofp`（codegen.rs:1749-1764）——同一个 helper 二选一，判据是
`slot_read_ids`（声明 :60，填充 :1635，消费 :1755，语义注释 :1746-1748/:1632）。
...
锚点 codegen.rs:4053-4061。容器真值另有一套按长度的判定（批次 297），
两者不互相转换 —— "非空 dict" 是 `len>0`，不是 `!= 0`。

**R6 struct 字段一律 64 位槽；宽度≠8 的类型进/出槽各 bitcast 一次。**
锚点 codegen.rs:6533（存侧注释）+ #2 读侧。
...
slot is int`（锚点 codegen.rs:3603 调用点、codegen.rs:7351 实现），并在 `report_abi_coercions`
...
`_`→`i64`）；返回映射 :1518-1522 + `infer_fn_return_type` :1392-1405（同样只在
f32/f64/i64 中选）。
...
:6556-6557）；元组走 `StackArray` 的 `[cap|len|elems…]`、句柄 = `buf+16`（:6740-6745、
:7126-7127，与 §1#6 同一几何）。读侧：字段访问 = `int_to_ptr` + **整块 load**
`struct_type(&[i64; N], false)`（:6501-6509，非 packed）+ `extract_value`（:6856-6859）；
字段写 = 地址算术 `base + idx*8` 后 `store`（:5717-5738）。
⚠️ **另一套通用映射器不参与签名**：`type_to_llvm_type`（:7734）会把 `Type::Tuple`
映成真 LLVM struct（:7840-7846）。改签名时只认 :1467-1480，认它就等着 ABI 不一致。
...
- **定义侧**（LLVM 签名）= `infer_fn_return_type`（:1392-1405）扫 `mir.stmts`，取
  **首条顶层 `Return`** 值在 `type_map` 里的类型，扫不到默认 `i64`（:1412）。
  ...
  ——这才是 :4845-4846 注释"nested ones are never consulted"的确切适用面。
...
> 锚点源码：src/backend/codegen/codegen.rs
**C3 补偿发生在返回语句处：`ret_sitofp` / `ret_fptosi`（:4823-4857）。**
读的是**当前正在生成的函数自身**的 LLVM 签名
（:4813-4822 `builder.get_insert_block().get_parent()` → `get_return_type()`），
...
:4709（void 调用兜底）、:5020（`DictInsert`→`map_insert`，传空 `arg_ids`）。

> 锚点源码：src/backend/codegen/codegen.rs
| # | 实参 → 形参 | 发出的指令 | 丢值？ | 告警 | 判定 |
|---|---|---|---|---|---|
| 1 | `i<n>` → `i<m>`，n<m | `arg_zext`（:7235-7242） | 否 | 无 | **保留** |
| 2 | `i<n>` → `i<m>`，n>m | `arg_trunc`（:7252-7255） | **是**（丢高位） | `narrow iA→iB`（:7243-7251） | **删除候选**（改诊断） |
| 3 | 同宽整数 | 原样（:7256-7258） | 否 | 无 | 保留 |
| 4 | int → float，且该实参来自容器裸槽 | `arg_slot_bits` **bitcast**（:7260-7271） | 否 | 无 | **保留**（与 §2 R2 对称，批次 299；判据 `slot_read_ids`） |
| 5 | int → float，其他来源 | `arg_sitofp`（:7272-7275） | 否 | 无 | 保留，但**应告警化**（值对、静默错） |
| 6 | float → int | `arg_fptosi`（:7287-7290） | **是**（小数；负数/OOB 为 LLVM poison） | `fptosi → iN`（:7281-7286） | **删除候选** |
| 7 | float ↔ ptr | **不转换**，原值照推（:7292-7296） | 值不匹配照样进 `build_call` | `float↔ptr (forbidden)` | **必改**：写着 forbidden 却不拒绝 |
| 8 | 其余一切（**含 ptr ↔ i64**） | 原样（:7297 `_ => push`） | 未知 | 无 | **删除候选**（今天的默认＝硬塞） |
| 9 | 实参少于形参 | 补 null/零常量（:7300-7321） | 静默补空 | 无 | **删除候选**（Python 侧应报错） |
| 10 | 实参多于形参 | `result.truncate(n_params)`（:7322-7323） | **静默丢实参** | 无 | **删除候选** |
...
`ptr` 进出，故调用点前对**第 0 个实参**插 `inttoptr`（:4609-4620 名单 + :4396-4404），
调用后对返回插 `ptrtoint`（:4646-4654）。
...
`printf`（:1127-1128）、`::` 限定名 extern 兜底桩（:3018-3022）。

### 3.4 间接调用与 Python 式形参折叠

**C8 闭包 V1 不捕获环境**：合成具名函数 `__closure_N` 直调，无环境结构体
（gen.rs:10498-10508 注释、:14351-14356 "The closure value is then the function address
(i64)"）。
...
分派守卫 gen.rs:11209-11233：`receiver.is_none()` 且 `arg_ids.len() <= 4` 且名字不是
...
gen.rs `fill` 按**声明顺序**落槽：位置实参（:8984-8990）→ 关键字（:8991-8996）→
`**` 映射填未绑定槽（:8997-9009）→ 默认值（:9010-9019）。
C 侧 `zeta_param_default` 是**恒等 no-op**（py_additions.c:2672），存在只为让标记不成未定义符号。
> 锚点源码：src/middle/mir/gen.rs
现状记录（不是愿望）：**重复绑定静默后者覆盖**（:8993 `slots[i] = Some(v)` 无检查）、
**未知关键字变成多余位置实参**（:8994 `slots.push`）→ 再由 §3.2#10 静默裁掉；
...
静态展开（gen.rs:9149-9185，"V1: static-size arrays compile-time unrolled"）；
日志调用里的 starred 实参明确**不展开**并告警（gen.rs:6581-6602）。
...
MIR 打类型 `Type::Named("PyArgNS")`（gen.rs:6204-6216）、消费端按字段访问分派到
`py_argparse_get_{i64,f64,bool,str}`（gen.rs:12318-12351）。
...
锚点 resolver.rs:2011、:2822；mir/gen.rs:402、:645、:757、:764、:3891。
构造就是两次 `replace`，**没有转义**：`__` 既是分隔符又可能出现在 member 里，
点号也会把 `a.b` 和 `a__b` 映到同一串。判"这是不是模块限定名"目前只有
`actual_name.contains("__")`（codegen.rs:3047，与同一行的
...
**N3 重载消歧形是 `<name>_<实参数>`，由 MIR 生成侧加**：gen.rs:11498、:13059
（`format!("{}_{}", func, arg_ids.len())`）。同处注释分三段：
加后缀的理由（src/middle/mir/gen.rs:11472-11474）、三类**不加**后缀的名字
（`zeta_*`、含 `__`、含 `::`，src/middle/mir/gen.rs:11475-11485）、以及"读侧剥后缀会误伤
名字自带的下划线"的现场记录（src/middle/mir/gen.rs:11486-11492：`DataFrame::reset_index`
被剥成 `DataFrame::reset`，返回类型查不到 ⇒ 结果 typed I64 ⇒ `b.columns` 成了一次假字段读）。
**读侧要靠剥后缀还原** ⇒ N3 的代价全在 §4.2 的瀑布里。

**N4 `str_*` 方法在符号层重写成 `host_str_*`**：codegen.rs:2692-2698
...
| 档 | 前提 | 尝试的名字 | 锚点 | 判性 |
|---|---|---|---|---|
| 1 | 名含 `::` | 精确限定名原样 | :2702-2710 | **承重墙**：注释原文要求"先试精确形，**不要**退回 mangle/裸方法名"——退回过，`a.column("code")[1]` 把一个 Vec 当 map 索引，SEGV（:2664-2668） |
| 2 | 名含 `::` | `::`→`__` | :2664-2670 | 承重墙（N6 的双态） |
| 3 | 名含 `::` | 裸方法名 + 实参数校验 | :2757-2765 | 事故现场：跨模块同名方法可被匹配（附 B#5′） |
| 4 | 名含 `::` | 裸方法名 + `_N` | :2766-2771 | 同 3 |
| 5 | — | `host_*` | :2773-2781 | N4 |
| 6 | — | 裸名 + 实参数校验 | :2782-2794 | 承重墙 |
| 7 | — | `name_N` | :2795-2803 | N3 的反向 |
| 8 | `type_args` 空 | 泛型默认实例化（参数全按 i64） | :2804-2811 | 承重墙 |
| 9 | `type_args` 非空 | `_inst_` mangle | :2813-2816 | N2 |
| 10 | 同上 | 剥 `_<数字>` 后 monomorphize | :2817-2830 | N3 反向 |
| 11 | 同上 | 剥尾缀再 `_inst_` | :2831-2843 | 注释点名 `oneshot::channel_0` |
| 12 | 同上 | 只用方法名 mangle（含再剥尾缀） | :2844-2863 | 事故现场 |
| 13 | 同上 | 限定路径 mangle + `::`→`__`（含再剥尾缀） | :2864-2888 | 事故现场 |
| 14 | 同上 | `host_*` / 裸名 | :2889-2898 | — |
| 15 | 汇流 | `name.<N>`（LLVM 改名） | :2900-2905 | 轴二，见 N8 |
| 16 | 汇流 | `name_N` 再试 | :2906-2913 | — |
| 17 | 汇流 | 剥尾缀 `_N` 试基名（**必须全数字**） | :2829-2855 | 承重墙：护栏注释原文——没有它 `to_string_f64` 曾被当成 `to_string`+后缀而静默改名（:2921-2923） |
...
锚点 codegen.rs:3060-3063 —— 兜底 extern 的签名一律是 `i64(i64, i64, …)`（参数个数 = 本调用点
...
存储侧写 `__`、调用点引用带 `::`（:2689-2690 注释原文）。`name.replace("::", "__")` 实测有
**9 处**（旧文档记"4 处"是错的，且它列的第 4 个锚点其实是一行注释）：
读侧查表 :2325、:2548、:2583、:2599、:2711，mangle 之后再拼 :2871、:2881，
extern 声明 :2956、:2959——**9 处全部落在 codegen.rs 这一份文件里**（全仓 grep 无第 10 处）。
...
读侧档位 codegen.rs:2900-2905；别名表 runtime/aliases.inc.c —— **恰好 66 条 `.set`**
（本批实测计数；行 4-69），形如
...
（:3031-3049 命中即直接复用已有声明；豁免判据在 :3047）。注释列了豁免前真实产生的 4 个不可满足符号
（`__get_price_3`/`__get_price_7`/`__get_trade_days_2`/`__OrderCost_6`，:3044-3046）——
...
（gen.rs:9706 `func: "[dynamic]str__map"`）；C 侧的实现叫 `zt_dyn_str_map`，
...
且已经有只造了名、没注册实现的变体（gen.rs:9602、:9621、:9640、:9659、:9719、:13733 注释里的
...
| ② gen.rs 分发 | 手工 | 例 gen.rs:9706、:9958 |
...
| `zeta_dyn_getitem` | :3427 | `base` 是 **L2 数据指针**或 **L3 map 句柄**；`key` 是索引，**负数按 `+len` 回卷**（:3433）；其 vec 判据写作 `cap >= 8`（:3432）⚠️ **与 L2 的 `cap >= 1`（:3474）不一致，且本批实测这是一个可观测的死循环**（`[1,2]+[3,4,5]` 的 cap=5 走不进 vec 臂 ⇒ 落到 `map_get` 的开放寻址环，`&(cap-1)` 在 cap=5 上不是掩码 ⇒ 永不停止；详见附 B#7） | gen.rs:13705；**手写**声明 codegen.rs:1092（2 参） |
| `zeta_dyn_len` | :3492 | 句柄**或**文本指针，**无标签**；读序 map→vec→文本（R4） | gen.rs:7276 |
| `zeta_dyn_contains` | :3510 | 4 元：容器 + 原键 + **map 归一键**（`map_str_key` 之值）+ `key_is_str` 选内容相等（:3506 原文） | gen.rs:9958 |
| `zeta_dyn_truth` | :3534 | map→已用槽数、vec→头长度、文本→首字节（:3529 原文）；文本分支先过 L7 的地址闸门（:3539） | gen.rs:4214 |
...
5. **名字编码不可逆（§4.1 的根因）**：N1/N2/N3 三种拼写共用 `_` 与 `__`，
   **没有任何转义规则**，于是 `<module>__<member>`、`<base>_inst_<T>`、`<name>_<arity>`
   ...
   读侧唯一的判形手段是 `contains("__")`（codegen.rs:3047）——它只回答"有没有"，
...
10. **`official: 194/194` 里的用例程序被解析器就地截断**（批次 320 收门禁 stderr 时
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **修法归属**：G.1/G.3（解析器 + 用例重构），不在 ABI 范围；优先级按上表丢行量。
     ...
     （`src/middle/mir/gen.rs:8873`），而 JIT 表里只有 `println_i64` ⇒ 新解析出的代码
   ...
   - **批次 322 关掉第二族：字符串字面量作 match 模式**（`parse_lit` 只吃数字，
     ...
     **五条 expect 全部落到 `_ =>`**。原因：`==` 在 IR 里被声明为
     ...
     （与 `op == "+"` 同形，后端 `codegen.rs:6215` 有 str 比较分支）。
   ...
   - **批次 323 关掉第三族：`s[i..]` 开区段下标**（任务 #39）。两层：
     ...
     **为何不在 `parse_expr` 里修**：优先级链是
     ...
     会当场崩编译器：`src/backend/codegen/codegen.rs:6532` 无条件索引 `exprs[field_id]`。
     触发链 `tests/unit-tests/minimal_compiler.z:129` → 崩点 `codegen.rs:6532`（rc=101）。
     修法：`Box::new` 走**恒等**下型（`Box<T>` 槽与其内值同为 64 位句柄，
     `src/middle/mir/gen.rs:12958`），`String::new()` 下成空串字面量（`:12962`；
   ...
   - **批次 325 推进第五族：范围模式（`'a'..='z'` / `1..=10` / `x @ 1..=10`）——
     ...
     **从没进过 `exprs`**，而 `gen_expr_safe`（`src/backend/codegen/codegen.rs:5917`）
     ...
     一行输出都没有**。修法是把 guard 的 dest 直接交给上层读（`gen.rs:12057`）。
     ④ `inclusive` 字段此前被 `inclusive: _` 丢掉 ⇒ `..` 一直按 `..=` 运行。
     现已区分（`gen.rs:3638`）。official 194 文件里**没有一处**用排他范围作模式
