...
| 2 | `f64` | **IEEE-754 位模式**，不是裸整数 | ① 静态 `double` 槽：`Assign` 时 int 值走 `slot_sitofp`（codegen.rs:3511-3536）；② struct 字段是 64 位槽：存前 bitcast（codegen.rs:6426） | 从**裸容器字**读回必须是 `bitcast`，不能 `sitofp`（`slot_or_sitofp` codegen.rs:1738-1753，判据 `slot_read_ids` 60 / 1593） | 批次 298：裸存 → 读回 4.94e-322；批次 299：缺 `sitofp` → `500.0 + 50` 算成 `4.6e18`（5782 记录） |
...
slot is int`（锚点 codegen.rs:3545 调用点、codegen.rs:7209 实现），并在 `report_abi_coercions`
...
:6449-6450）；元组走 `StackArray` 的 `[cap|len|elems…]`、句柄 = `buf+16`（:6740-6745、
:6984-6985，与 §1#6 同一几何）。读侧：字段访问 = `int_to_ptr` + **整块 load**
`struct_type(&[i64; N], false)`（:6501-6509，非 packed）+ `extract_value`（:6714-6717）；
字段写 = 地址算术 `base + idx*8` 后 `store`（:5659-5680）。
⚠️ **另一套通用映射器不参与签名**：`type_to_llvm_type`（:7592）会把 `Type::Tuple`
映成真 LLVM struct（:7698-7704）。改签名时只认 :1467-1480，认它就等着 ABI 不一致。
...
> 锚点源码：src/backend/codegen/codegen.rs
锚点 codegen.rs:7059-7184；告警器 `abi_note` :7154-7169（stderr 最多 8 条，
...
|---|---|---|---|---|---|
| 1 | `i<n>` → `i<m>`，n<m | `arg_zext`（:7093-7100） | 否 | 无 | **保留** |
| 2 | `i<n>` → `i<m>`，n>m | `arg_trunc`（:7110-7113） | **是**（丢高位） | `narrow iA→iB`（:7101-7109） | **删除候选**（改诊断） |
| 3 | 同宽整数 | 原样（:7114-7116） | 否 | 无 | 保留 |
| 4 | int → float，且该实参来自容器裸槽 | `arg_slot_bits` **bitcast**（:7118-7129） | 否 | 无 | **保留**（与 §2 R2 对称，批次 299；判据 `slot_read_ids`） |
| 5 | int → float，其他来源 | `arg_sitofp`（:7130-7133） | 否 | 无 | 保留，但**应告警化**（值对、静默错） |
| 6 | float → int | `arg_fptosi`（:7145-7148） | **是**（小数；负数/OOB 为 LLVM poison） | `fptosi → iN`（:7139-7144） | **删除候选** |
| 7 | float ↔ ptr | **不转换**，原值照推（:7150-7154） | 值不匹配照样进 `build_call` | `float↔ptr (forbidden)` | **必改**：写着 forbidden 却不拒绝 |
| 8 | 其余一切（**含 ptr ↔ i64**） | 原样（:7155 `_ => push`） | 未知 | 无 | **删除候选**（今天的默认＝硬塞） |
| 9 | 实参少于形参 | 补 null/零常量（:7158-7179） | 静默补空 | 无 | **删除候选**（Python 侧应报错） |
| 10 | 实参多于形参 | `result.truncate(n_params)`（:7180-7181） | **静默丢实参** | 无 | **删除候选** |
...
| M4 | capybara COMPILER_BUGS #4 的复现：`fn get_pair() -> (i64, i64): return (42, 99)` + `let (a, b) = get_pair()` | `42 / 99`，rc=0 | **该 bug 今天不再复现**；其"返回局部指针"根因假设（COMPILER_BUGS.md:39、NOTES.md:25-28）被 M1/M4 证伪 —— 写侧本就 heap 分配（codegen.rs:6211、RELEASE_NOTES_v1.0.19.zeta:19）。"garbage fields"的**现行**来源是字段数解析失败时的 `("", 2)` 二字段兜底（codegen.rs:6399-6404 注释即记此事、兜底点在 codegen.rs:6647-6650） |
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **批次 322 关掉第二族：字符串字面量作 match 模式**（`parse_lit` 只吃数字，
     ...
     **五条 expect 全部落到 `_ =>`**。原因：`==` 在 IR 里被声明为
     ...
     （与 `op == "+"` 同形，后端 `codegen.rs:6108` 有 str 比较分支）。
   ...
   - **批次 323 关掉第三族：`s[i..]` 开区段下标**（任务 #39）。两层：
     ...
     **为何不在 `parse_expr` 里修**：优先级链是
     ...
     会当场崩编译器：`src/backend/codegen/codegen.rs:6425` 无条件索引 `exprs[field_id]`。
     触发链 `tests/unit-tests/minimal_compiler.z:129` → 崩点 `codegen.rs:6425`（rc=101）。
