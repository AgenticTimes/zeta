...
前提（codegen.rs:1603-1607）：**静态可定的本地按自身 LLVM 类型开槽**
...
| 2 | `f64` | **IEEE-754 位模式**，不是裸整数 | ① 静态 `double` 槽：`Assign` 时 int 值走 `slot_sitofp`（codegen.rs:3345-3370）；② struct 字段是 64 位槽：存前 bitcast（codegen.rs:6260） | 从**裸容器字**读回必须是 `bitcast`，不能 `sitofp`（`slot_or_sitofp` codegen.rs:1710-1725，判据 `slot_read_ids` 60 / 1593） | 批次 298：裸存 → 读回 4.94e-322；批次 299：缺 `sitofp` → `500.0 + 50` 算成 `4.6e18`（5782 记录） |
| 3 | `bool` | **i64 的 0/1** | 比较结果立刻 `zext i1→i64`（`eq_ext`/`ne_ext`/…，codegen.rs:3829-3837） | 一律 `!= 0` | `Type::Bool → bool_type`（7388）只在 ABI 边界（参数/返回签名）出现；**i1 从不进槽** |
| 4 | `str`（指针形） | `.rodata` 或 GC 块首地址，`ptrtoint` 成 i64 | `MirExpr::StringLit`：私有 global 字节数组 + `ptrtoint`（codegen.rs:5754-5773） | 可直接解引用；`GC_base(h) != 0` ⇒ 堆串 | 与 #5 不可静态区分 ⇒ 见 R3 |
...
锚点 codegen.rs:3345-3370（`MirStmt::Assign` 的 `slot_sitofp`）。
...
锚点 `slot_or_sitofp`（codegen.rs:1710-1725）——同一个 helper 二选一，判据是
`slot_read_ids`（声明 :60，填充 :1601，消费 :1716，语义注释 :1707-1709/:1632）。
...
锚点 codegen.rs:3829-3837。容器真值另有一套按长度的判定（批次 297），
两者不互相转换 —— "非空 dict" 是 `len>0`，不是 `!= 0`。

**R6 struct 字段一律 64 位槽；宽度≠8 的类型进/出槽各 bitcast 一次。**
锚点 codegen.rs:6260（存侧注释）+ #2 读侧。
...
slot is int`（锚点 codegen.rs:3379 调用点、:7035 实现），并在 `report_abi_coercions`
...
一个格子只装一个 64 位词）。**函数体里没有这个名的槽时，读走 env**（gen.rs:3596 起），
所以只写局部槽的写路径等于把这次赋值丢掉。三条写路径现在是这样：
① `=` —— 写局部槽 + 镜像 env（gen.rs:3596 起，本规则的第一处实现）；
...
**三条路径镜像的都是"刚写过的那个槽"，不是原来那条表达式**（`=`：gen.rs:3596 传
`existing`、gen.rs:3596 传 `new_id`；`+=`：gen.rs:3596 传 `slot_id`）。理由实测：折叠出的加法表达式里就带着这个槽
...
模块全局用的是那条 `=` 留下的局部槽 —— 读侧先看这个函数已有的槽（gen.rs:3452，在 gen.rs:3596
...
`_`→`i64`）；返回映射 :1484-1488 + `infer_fn_return_type` :1392-1405（同样只在
f32/f64/i64 中选）。
...
:6283-6284）；元组走 `StackArray` 的 `[cap|len|elems…]`、句柄 = `buf+16`（:6740-6745、
:6818-6819，与 §1#6 同一几何）。读侧：字段访问 = `int_to_ptr` + **整块 load**
`struct_type(&[i64; N], false)`（:6501-6509，非 packed）+ `extract_value`（:6548-6551）；
字段写 = 地址算术 `base + idx*8` 后 `store`（:5493-5514）。
⚠️ **另一套通用映射器不参与签名**：`type_to_llvm_type`（:7426）会把 `Type::Tuple`
映成真 LLVM struct（:7532-7538）。改签名时只认 :1467-1480，认它就等着 ABI 不一致。
...
- **定义侧**（LLVM 签名）= `infer_fn_return_type`（:1392-1405）扫 `mir.stmts`，取
  **首条顶层 `Return`** 值在 `type_map` 里的类型，扫不到默认 `i64`（:1412）。
  ...
  ——这才是 :4621-4622 注释"nested ones are never consulted"的确切适用面。
...
> 锚点源码：src/backend/codegen/codegen.rs
**C3 补偿发生在返回语句处：`ret_sitofp` / `ret_fptosi`（:4599-4633）。**
读的是**当前正在生成的函数自身**的 LLVM 签名
（:4589-4598 `builder.get_insert_block().get_parent()` → `get_return_type()`），
...
> 锚点源码：src/backend/codegen/codegen.rs
锚点 codegen.rs:6893-7018；告警器 `abi_note` :7012-7026（stderr 最多 8 条，
`strict_abi` 时首条即致命）。调用点：:3859、:3875（运算符名兜底）、:4410（常规调用）、
:4567（void 调用兜底）、:4796（`DictInsert`→`map_insert`，传空 `arg_ids`）。

> 锚点源码：src/backend/codegen/codegen.rs
| # | 实参 → 形参 | 发出的指令 | 丢值？ | 告警 | 判定 |
|---|---|---|---|---|---|
| 1 | `i<n>` → `i<m>`，n<m | `arg_zext`（:6927-6934） | 否 | 无 | **保留** |
| 2 | `i<n>` → `i<m>`，n>m | `arg_trunc`（:6944-6947） | **是**（丢高位） | `narrow iA→iB`（:6935-6943） | **删除候选**（改诊断） |
| 3 | 同宽整数 | 原样（:6948-6950） | 否 | 无 | 保留 |
| 4 | int → float，且该实参来自容器裸槽 | `arg_slot_bits` **bitcast**（:6952-6963） | 否 | 无 | **保留**（与 §2 R2 对称，批次 299；判据 `slot_read_ids`） |
| 5 | int → float，其他来源 | `arg_sitofp`（:6964-6967） | 否 | 无 | 保留，但**应告警化**（值对、静默错） |
| 6 | float → int | `arg_fptosi`（:6979-6982） | **是**（小数；负数/OOB 为 LLVM poison） | `fptosi → iN`（:6973-6978） | **删除候选** |
| 7 | float ↔ ptr | **不转换**，原值照推（:6984-6988） | 值不匹配照样进 `build_call` | `float↔ptr (forbidden)` | **必改**：写着 forbidden 却不拒绝 |
| 8 | 其余一切（**含 ptr ↔ i64**） | 原样（:6989 `_ => push`） | 未知 | 无 | **删除候选**（今天的默认＝硬塞） |
| 9 | 实参少于形参 | 补 null/零常量（:6992-7013） | 静默补空 | 无 | **删除候选**（Python 侧应报错） |
| 10 | 实参多于形参 | `result.truncate(n_params)`（:7014-7015） | **静默丢实参** | 无 | **删除候选** |
...
`ptr` 进出，故调用点前对**第 0 个实参**插 `inttoptr`（:4385-4396 名单 + :4396-4404），
调用后对返回插 `ptrtoint`（:4422-4430）。
...
`printf`（:1114-1115）、`::` 限定名 extern 兜底桩（:2910-2914）。

### 3.4 间接调用与 Python 式形参折叠

**C8 闭包 V1 不捕获环境**：合成具名函数 `__closure_N` 直调，无环境结构体
（gen.rs:10498-10508 注释、:13465-13470 "The closure value is then the function address
(i64)"）。
...
分派守卫 gen.rs:10576-10600：`receiver.is_none()` 且 `arg_ids.len() <= 4` 且名字不是
...
gen.rs `fill` 按**声明顺序**落槽：位置实参（:8490-8496）→ 关键字（:8497-8502）→
`**` 映射填未绑定槽（:8503-8515）→ 默认值（:8516-8525）。
C 侧 `zeta_param_default` 是**恒等 no-op**（py_additions.c:2672），存在只为让标记不成未定义符号。
> 锚点源码：src/middle/mir/gen.rs
现状记录（不是愿望）：**重复绑定静默后者覆盖**（:8499 `slots[i] = Some(v)` 无检查）、
**未知关键字变成多余位置实参**（:8500 `slots.push`）→ 再由 §3.2#10 静默裁掉；
...
静态展开（gen.rs:8654-8690，"V1: static-size arrays compile-time unrolled"）；
日志调用里的 starred 实参明确**不展开**并告警（gen.rs:6124-6145）。
...
MIR 打类型 `Type::Named("PyArgNS")`（gen.rs:5747-5759）、消费端按字段访问分派到
`py_argparse_get_{i64,f64,bool,str}`（gen.rs:11577-11610）。
...
| M4 | capybara COMPILER_BUGS #4 的复现：`fn get_pair() -> (i64, i64): return (42, 99)` + `let (a, b) = get_pair()` | `42 / 99`，rc=0 | **该 bug 今天不再复现**；其"返回局部指针"根因假设（COMPILER_BUGS.md:39、NOTES.md:25-28）被 M1/M4 证伪 —— 写侧本就 heap 分配（codegen.rs:6211、RELEASE_NOTES_v1.0.19.zeta:19）。"garbage fields"的**现行**来源是字段数解析失败时的 `("", 2)` 二字段兜底（codegen.rs:6399-6404 注释即记此事、兜底点在 codegen.rs:6481-6484） |
...
锚点 resolver.rs:1887、:2685；mir/gen.rs:392、:635、:747、:754、:3565。
构造就是两次 `replace`，**没有转义**：`__` 既是分隔符又可能出现在 member 里，
点号也会把 `a.b` 和 `a__b` 映到同一串。判"这是不是模块限定名"目前只有
`actual_name.contains("__")`（codegen.rs:2939，与同一行的
...
**N3 重载消歧形是 `<name>_<实参数>`，由 MIR 生成侧加**：gen.rs:10842、:11973
（`format!("{}_{}", func, arg_ids.len())`）。同处注释分三段：
加后缀的理由（src/middle/mir/gen.rs:10817-10819）、三类**不加**后缀的名字
（`zeta_*`、含 `__`、含 `::`，src/middle/mir/gen.rs:10820-10830）、以及"读侧剥后缀会误伤
名字自带的下划线"的现场记录（src/middle/mir/gen.rs:10831-10837：`DataFrame::reset_index`
被剥成 `DataFrame::reset`，返回类型查不到 ⇒ 结果 typed I64 ⇒ `b.columns` 成了一次假字段读）。
**读侧要靠剥后缀还原** ⇒ N3 的代价全在 §4.2 的瀑布里。

**N4 `str_*` 方法在符号层重写成 `host_str_*`**：codegen.rs:2653-2659
...
| 档 | 前提 | 尝试的名字 | 锚点 | 判性 |
|---|---|---|---|---|
| 1 | 名含 `::` | 精确限定名原样 | :2663-2671 | **承重墙**：注释原文要求"先试精确形，**不要**退回 mangle/裸方法名"——退回过，`a.column("code")[1]` 把一个 Vec 当 map 索引，SEGV（:2664-2668） |
| 2 | 名含 `::` | `::`→`__` | :2664-2670 | 承重墙（N6 的双态） |
| 3 | 名含 `::` | 裸方法名 + 实参数校验 | :2679-2687 | 事故现场：跨模块同名方法可被匹配（附 B#5′） |
| 4 | 名含 `::` | 裸方法名 + `_N` | :2688-2693 | 同 3 |
| 5 | — | `host_*` | :2695-2703 | N4 |
| 6 | — | 裸名 + 实参数校验 | :2704-2716 | 承重墙 |
| 7 | — | `name_N` | :2717-2725 | N3 的反向 |
| 8 | `type_args` 空 | 泛型默认实例化（参数全按 i64） | :2726-2733 | 承重墙 |
| 9 | `type_args` 非空 | `_inst_` mangle | :2735-2738 | N2 |
| 10 | 同上 | 剥 `_<数字>` 后 monomorphize | :2739-2752 | N3 反向 |
| 11 | 同上 | 剥尾缀再 `_inst_` | :2753-2765 | 注释点名 `oneshot::channel_0` |
| 12 | 同上 | 只用方法名 mangle（含再剥尾缀） | :2766-2785 | 事故现场 |
| 13 | 同上 | 限定路径 mangle + `::`→`__`（含再剥尾缀） | :2786-2810 | 事故现场 |
| 14 | 同上 | `host_*` / 裸名 | :2811-2820 | — |
| 15 | 汇流 | `name.<N>`（LLVM 改名） | :2822-2827 | 轴二，见 N8 |
| 16 | 汇流 | `name_N` 再试 | :2828-2835 | — |
| 17 | 汇流 | 剥尾缀 `_N` 试基名（**必须全数字**） | :2829-2855 | 承重墙：护栏注释原文——没有它 `to_string_f64` 曾被当成 `to_string`+后缀而静默改名（:2843-2845） |
...
锚点 codegen.rs:2952-2955 —— 兜底 extern 的签名一律是 `i64(i64, i64, …)`（参数个数 = 本调用点
...
存储侧写 `__`、调用点引用带 `::`（:2650-2651 注释原文）。`name.replace("::", "__")` 实测有
**9 处**（旧文档记"4 处"是错的，且它列的第 4 个锚点其实是一行注释）：
读侧查表 :2278、:2501、:2536、:2552、:2664，mangle 之后再拼 :2793、:2803，
extern 声明 :2878、:2881——**9 处全部落在 codegen.rs 这一份文件里**（全仓 grep 无第 10 处）。
...
读侧档位 codegen.rs:2822-2827；别名表 runtime/aliases.inc.c —— **恰好 66 条 `.set`**
（本批实测计数；行 4-69），形如
...
（:2923-2941 命中即直接复用已有声明；豁免判据在 :2939）。注释列了豁免前真实产生的 4 个不可满足符号
（`__get_price_3`/`__get_price_7`/`__get_trade_days_2`/`__OrderCost_6`，:2936-2938）——
...
（gen.rs:9160 `func: "[dynamic]str__map"`）；C 侧的实现叫 `zt_dyn_str_map`，
...
且已经有只造了名、没注册实现的变体（gen.rs:9056、:9075、:9094、:9113、:9173、:12857 注释里的
...
| ② gen.rs 分发 | 手工 | 例 gen.rs:9160、:9412 |
...
| `zeta_dyn_getitem` | :3427 | `base` 是 **L2 数据指针**或 **L3 map 句柄**；`key` 是索引，**负数按 `+len` 回卷**（:3433）；其 vec 判据写作 `cap >= 8`（:3432）⚠️ **与 L2 的 `cap >= 1`（:3474）不一致，且本批实测这是一个可观测的死循环**（`[1,2]+[3,4,5]` 的 cap=5 走不进 vec 臂 ⇒ 落到 `map_get` 的开放寻址环，`&(cap-1)` 在 cap=5 上不是掩码 ⇒ 永不停止；详见附 B#7） | gen.rs:12829；**手写**声明 codegen.rs:1079（2 参） |
| `zeta_dyn_len` | :3492 | 句柄**或**文本指针，**无标签**；读序 map→vec→文本（R4） | gen.rs:6859 |
| `zeta_dyn_contains` | :3510 | 4 元：容器 + 原键 + **map 归一键**（`map_str_key` 之值）+ `key_is_str` 选内容相等（:3506 原文） | gen.rs:9412 |
| `zeta_dyn_truth` | :3534 | map→已用槽数、vec→头长度、文本→首字节（:3529 原文）；文本分支先过 L7 的地址闸门（:3539） | gen.rs:3843 |
...
5. **名字编码不可逆（§4.1 的根因）**：N1/N2/N3 三种拼写共用 `_` 与 `__`，
   **没有任何转义规则**，于是 `<module>__<member>`、`<base>_inst_<T>`、`<name>_<arity>`
   ...
   读侧唯一的判形手段是 `contains("__")`（codegen.rs:2939）——它只回答"有没有"，
...
10. **`official: 194/194` 里的用例程序被解析器就地截断**（批次 320 收门禁 stderr 时
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **修法归属**：G.1/G.3（解析器 + 用例重构），不在 ABI 范围；优先级按上表丢行量。
     ...
     （`src/middle/mir/gen.rs:8379`），而 JIT 表里只有 `println_i64` ⇒ 新解析出的代码
   ...
   - **批次 322 关掉第二族：字符串字面量作 match 模式**（`parse_lit` 只吃数字，
     ...
     **五条 expect 全部落到 `_ =>`**。原因：`==` 在 IR 里被声明为
     ...
     （与 `op == "+"` 同形，后端 `codegen.rs:5942` 有 str 比较分支）。
   ...
   - **批次 323 关掉第三族：`s[i..]` 开区段下标**（任务 #39）。两层：
     ① 解析：`src/frontend/parser/expr.rs:2292` 新增 `slice_sep`（`:` 与 `..` 二选一，
     且拒绝 `...` 的前两字符），`src/frontend/parser/expr.rs:2438` 的起始界分支改走它。
     **为何不在 `parse_expr` 里修**：优先级链是
     ...
     会当场崩编译器：`src/backend/codegen/codegen.rs:6259` 无条件索引 `exprs[field_id]`。
     触发链 `tests/unit-tests/minimal_compiler.z:129` → 崩点 `codegen.rs:6259`（rc=101）。
     修法：`Box::new` 走**恒等**下型（`Box<T>` 槽与其内值同为 64 位句柄，
     `src/middle/mir/gen.rs:12114`），`String::new()` 下成空串字面量（`:12118`；
   ...
   - **批次 325 推进第五族：范围模式（`'a'..='z'` / `1..=10` / `x @ 1..=10`）——
     ...
     **从没进过 `exprs`**，而 `gen_expr_safe`（`src/backend/codegen/codegen.rs:5693`）
