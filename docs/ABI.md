...
分派守卫 gen.rs:10834-10858：`receiver.is_none()` 且 `arg_ids.len() <= 4` 且名字不是
...
> 锚点源码：src/middle/mir/gen.rs
现状记录（不是愿望）：**重复绑定静默后者覆盖**（:8723 `slots[i] = Some(v)` 无检查）、
**未知关键字变成多余位置实参**（:8724 `slots.push`）→ 再由 §3.2#10 静默裁掉；
...
静态展开（gen.rs:8878-8914，"V1: static-size arrays compile-time unrolled"）；
日志调用里的 starred 实参明确**不展开**并告警（gen.rs:6315-6336）。
...
MIR 打类型 `Type::Named("PyArgNS")`（gen.rs:5938-5950）、消费端按字段访问分派到
`py_argparse_get_{i64,f64,bool,str}`（gen.rs:11943-11976）。
...
名字自带的下划线"的现场记录（src/middle/mir/gen.rs:11111-11117：`DataFrame::reset_index`
...
（gen.rs:9418 `func: "[dynamic]str__map"`）；C 侧的实现叫 `zt_dyn_str_map`，
...
| ② gen.rs 分发 | 手工 | 例 gen.rs:9418、:9670 |
...
10. **`official: 194/194` 里的用例程序被解析器就地截断**（批次 320 收门禁 stderr 时
   ...
   **后果**：① 官方基线对这批文件只覆盖了程序前缀，"194/194"不能读成
   ...
   - **修法归属**：G.1/G.3（解析器 + 用例重构），不在 ABI 范围；优先级按上表丢行量。
     ...
     （`src/middle/mir/gen.rs:8603`），而 JIT 表里只有 `println_i64` ⇒ 新解析出的代码
   ...
   - **批次 325 推进第五族：范围模式（`'a'..='z'` / `1..=10` / `x @ 1..=10`）——
     ...
     **从没进过 `exprs`**，而 `gen_expr_safe`（`src/backend/codegen/codegen.rs:5693`）
     ...
     一行输出都没有**。修法是把 guard 的 dest 直接交给上层读（`gen.rs:11682`）。
