...
```
主线批次（298/299 → final_value 对齐）
   │  提交间隙 / 只读诊断可并行            │  主线收尾后
   ├─ 轴A 死代码减法（blockchain ✅）       ├─ T0 mir_diff.sh ─► B/T1 掩码语义
   ├─ §5.2 known-fail 包                   │        └─► B/T2 + F.2 签名表/符号表（合并）
   ├─ §5.3/5.4 契约+告警                   │              ─► F.1-F.4 类型检查器（产 PyDynamic 边界清单）
   ├─ G.1 -O3 诊断（只读：复现+MSan+opt二分）                    ─► B/T3-T5 值模型装箱（边界来自 F）
   ├─ G.2 t228+ASan 常态化
   ├─ G.5 ABI 合同（docs/ABI.md，文档+收敛）
   ├─ G.7a/G.8a 截断与假值桩报告化（零行为变更，冷文件）    └─ 行为变更协调位：G.1 -O3 修复 / G.7b 同步恢复
   │                                                          / G.8b 桩响亮化（可能翻出门禁红或崩主线驱动）→ G.4 降级点清零
   ├─ G.3 差分 harness 起步（新文件零冲突）
   └─ 性能基线测量 ──────────────────────────────► 轴C intern/FxHashMap ─► §4.1 gen.rs 拆分
```
...
| B/T1 掩码语义 | 停掉一类猜坑 | 1 天 | 低 | 主线后（同类坑复发可提前） |
| B/T2 + F.2 签名表 | 跨函数类型地基 | 2 天 | 中 | 主线后 |
| F.1-F.4 类型检查器 | 定型三处收敛为一处；边界清单产出 | 5-6 天 | 中（每步门禁） | F 之后 |
| B/T3-T5 值模型 | 终止换坑式批次 | 5~7 天 | 中（探针兜底渐进） | F 之后（边界来自 F） |
