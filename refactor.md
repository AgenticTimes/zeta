...
| **B 类型标签战役** | 统一值模型：值不带类型 ⇒ 运行期探针族 + 编译期侧信道墙 | 见 §2 | 未动（**核心**） |
...
| **⑪ 性能基线是幽灵门禁** | — | `.github/workflows/benchmarks.yml:106/:148` 要 `--bench compiler_bench`/`runtime_bench` 但 **`benches/` 目录不存在**；`:190/:199/:209` 要 `--bin regression_test` 但 **`src/bin/regression_test.rs` 不存在**；`Cargo.toml:94` 有 criterion 依赖、全仓无 `[[bench]]` 段 ⇒ 依赖付了 harness 没接；该 workflow 每天 02:00 UTC 定时无条件失败 | 🟡 **批次 313 走仓内脚本路线落地**：`tools/perf_baseline.py`（不碰 `Cargo.toml`）。两阶段口径 `aot`(端到端，含 clang 链接) / `ir`(`--emit-llvm`，轴 B/C 改 IR 体量最灵敏)； **判定默认只压 `ir` 的 total**（实测 5 次独立调用散布 3.5%），`aot` 跨调用漂 20.7% 只作参考， 且必须先暖机（冷/热差会把基线读成 +38% 假回退）——三个口径全是量出来的； 带 `--calibrate` 复测噪声地板。基线：`ir` 41,655 ms / `aot` 30,186 ms（12 文件， 前 3 大文件占 83%）。⬜ CI 侧 `[[bench]]`+`harness=false` 仍待 `Cargo.toml` 解锁； ⬜ aot 侧要变可判需 A/B 交错（任务 #27） |
...
`runtime/py_additions.c` 的探针族。每个条目都有一段"第一版 SIGSEGV → 改判形"的踩雷史
（注释即证据），**它们不是设计，是幸存者**：
...
| B GC 块头加 tag 字 | 每个容器块头塞一个 tag word | 根治（容器） | 中 | map 扩容转发（负 cap）、vec `base-16` 头假设遍布 runtime，动块头=全库地雷 | 不选 |
...
- 现状：`MIR_SNAPSHOT_RUNS=3` 下 34/39 文件逐字节稳定；余 5 个（wufu 家族 + 驱动）
...
2. **known-fail 机制转正**：已知坏形态（元组变量 `in`、`df[bool_vec]` 零覆盖等）用
   ...
   **进度（批次 304）**：✅ 已落地，但**先修了机制**——旧路径只看"编译 rc==0"，
...
> ✅ **诊断档落地 2026-09-21（批次 307）**——只读，新增 `tools/opt_matrix.sh`。结论比
> 原设想更尖锐，先记事实：
>
...
>
> **验收口径修正**：原计划"OPT_LEVEL 矩阵全绿进 CI"里，"全绿"当前定义为
...
> ✅ **部分落地 2026-09-21（批次 306）**——工具与清单已就位，但**覆盖范围比原设想小**，
...
>    但第 2 条的盲区使"堆内越界/未初始化读"**未**被排除 ⇒ 降级为"G.2b 落地后再判"，
>    不再占用还债首位。
...
> **批次 319 起，#33 从"待修"变成"修了一半"**：R7 的返回侧诊断已落地（(a) 完成、
...
| 章 | 完备度 | 已有材料（可收割进 docs/ABI.md） | 缺口 |
|---|---|---|---|
...
| §3 调用约定 | ✅ **已成文（批次 316，批次 318 更正 C2/C3 + 补 M5–M7）** C1–C12 + 强转全表 + §3.5 七个实测探针 | ~~`zeta_call1` 两行注释~~ → 现在是 §3.3/§3.4 的成文规则；validate.md 的 abs(-2.5)→nan 批评已落到 §3.2 表 #6 | ~~返回类型的最终裁决层未定位（附 B#4）~~**已定位并关闭（批次 318）：结论是"没有裁决层"**——被调方签名只来自 `infer_fn_return_type`，声明 `-> T` 只到调用方 dest 槽，二者无一致性检查（`struct Mir` 无 `return_type` 字段）。**顺带推翻批次 316 的 M2"声明类型赢"**（实为位型重解释，M5 打印 `4612811918334230528`）。残余缺口从"未定位"变成"已定位但未修"⇒ 任务 #33；强转全表的"删除候选"落地 = G.5d |
...
> ✅ **已落地 2026-09-21（批次 316，commit 1f78d9d7）**——docs/ABI.md §3.1–§3.5
...
> ✅ **已落地 2026-09-21（批次 317，commit d004230e，任务 #31）**：`docs/ABI.md`
...
- **§4 名字修饰**：`mod__name`、`::` vs `__` 双态（capybara #3）、
  `_[dynamic]str__map` 魔法符号——逐条标注"承重墙"还是"事故"；**收割
...
- **输入已由 G.5b/c 备好（批次 316/317），逐条对应**：
  ...
  ② ~~附 B#4"返回类型最终裁决层未定位"——先定位再给返回侧补一条与 §3.2 对称的告警~~
     **定位已完成（批次 318，附 B#4 关闭）：结论是"根本没有裁决层"**，
     ...
     (a) ✅ **已落地（批次 319）**：与 §3.2 对称的**返回侧告警**（不改行为、只出声，
  ...
  ⑧ ✅ **工具已落地（批次 320 / 任务 #35）**：`codegen.rs` 一插行，ABI.md 的锚点全漂移
...
- **G.7a 截断响亮报告（立即，0.5 天，行为零变更）**：解析器在丢弃输入处打 stderr 告警
  ...
  **进度（2026-09-21 实测盘点）**：告警侧已落地 —— `src/main.rs` 的
  ...
  **1805 行被丢**，前三名 `minimal_compiler.z` 757 · `benchmark_simd_vs_scalar.z`
  357 · `selfhost.z` 158。G.7b 的范围从此逐文件可核，不用估。✅ **报告侧已落地**
...
- **G.8a 桩标记 + 调用报告（立即，0.5 天，行为零变更，冷文件）**：registry 条目加
  ...
  **进度（批次 303）**：✅ **已落地**。`--list-stubs` 给 18 个标记（registry 1 +
...
| 还债项 | 内容 | 归并到 |
|---|---|---|
| ① t228 非确定性专项 | 20 连跑 + dump-mir diff + ASan（0.5~1 天） | **§7 G.2**（升级为 sanitizer 常态化）。批次 306 实测：200 连跑 0 失败、MIR 20 次编译字节稳定、ASan 20 轮零命中 ⇒ 今天复现不出；残留嫌疑恰是 ASan 在 GC 堆上的盲区 ⇒ **降为"G.2b 落地后再判"，退出还债首位** |
...
| ④ gen.rs 拆分第一刀 | mir_diff.sh 前置 + 三刀（1~2 天） | 本计划 T0 + §4.1 |
...
```
债务清理期（backlog.md 燃尽——✅ 立即档 9/11、A 族清空、C 族过半、19+ 项关闭）
   │  剩余：B 族截断（#36 缩到 match 族先行）+ S 项并批 + D 族收尾
   ▼
【最小架构弧】（2026-09-23 用户裁定插入，4-6 天——在主线 301 恢复之前）
   ├─ ① B/T1 掩码语义（1 天）
   ├─ ② B/T2 + F.2 跨函数签名表（1-2 天）
   └─ ③ F 核心：检查器最小版——未标注=PyDynamic + PyDynamic 边界清单（2-3 天）
   ▼
主线批次 301 恢复（0 成交根因 → universe/parquet 分歧 → final_value 对齐，约 2-4 天）
   │  带着类型检查器修主线：类型洞从"三层各打补丁"变成"签名表加一行"
   ▼ 主线收尾后（final_value 对齐 = 正确性标杆已到手）
   ├─ F.4 全量迁移（拆除 parser/MIR/codegen 三处定型特判）
   ├─ B/T3-T5 值模型装箱（边界清单来自 F；探针逐族退役）
   ├─ gen.rs 拆分（还债④）+ 轴C intern/FxHashMap
   └─ 行为变更协调位：G.1 修复 / G.7b / G.8b → G.4 降级点清零
```

| 项 | 收益 | 成本 | 风险 | 时点 |
|---|---|---|---|---|
| 轴A 死代码 | -1.6 万行、依赖图缩小 | 0.5~1 天 | 低（逐项 grep 复核） | **立即** |
| §5.2 known-fail 包 | 堵复发 | 半天 | 无（不破门禁） | ✅ **已落地 2026-09-21（批次 304）** |
| T0 mir_diff.sh | 一切 MIR 重构的安全网 | 半小时 | 无 | ✅ **已落地 2026-09-21（批次 302）** |
| G.1 诊断（只读） | 定位 -O3 UB 真因 | 1 天 | 无（不动热文件） | ✅ **诊断已落地 2026-09-21（批次 307）**：结论是**方向相反**——今天没有"-O3 编错"的现行，反倒是 2 例只有 -O3 才对（`t06` 调用无定义的 `@Color__Green`、`t31` -O0 立即 SIGBUS）；假设 2（TBAA/属性）静态否证。修复档（单变量旋钮 + 这两例）待与主线协调 |
| G.2 sanitizer + t228 | 抓内存问题现行 | 0.5~1 天 | 低 | ✅ **部分落地 2026-09-21（批次 306）**：C 侧插桩 + 命中清单 + 阳性/盲区自对照；**生成码侧未做**（需 LLVM ASan pass，前置=轴 B/F），GC 块内越界另立 **G.2b**（canary） |
| G.5 ABI 合同（G.5a-e） | 同类 ABI bug 绝迹的制动器；审计=§3 接近零、§4 有分析无合同 | a-d 2~3 天；e 符号注册表 1-2 周单列 | 低（a-c 纯文档）/ 中（d、e） | ✅ **a 已落地 2026-09-21（批次 305）** / 其余**立即**（e 排后） |
| G.6 MIR verifier | 每 pass 合法性闸门 | 1~2 天 | 低-中（新模块） | 随批次穿插（T0 后即可） |
| G.7 解析静默截断（G.7a/b） | 静默吃代码绝迹 | a 0.5 天；b 1-2 天 | 低（a 零行为变更）/ 中（b 翻出真缺口动门禁） | ✅ **a 已落地 2026-09-21（批次 303）** / 协调（b） |
| G.8 假值桩响亮化（G.8a/b） | fail-loud 红线落实 | a+b 各 0.5 天 | 低（a 零行为变更）/ 中（b 可能崩主线驱动） | ✅ **a 已落地 2026-09-21（批次 303）** / 协调（b） |
| G.3 差分 harness | 语义一致性可度量 | 2 天起步 | 无（新文件） | **立即起步** |
| B/T1 掩码语义 | 停掉一类猜坑 | 1 天 | 低 | **最小架构弧①**（清理期后、主线 301 前） |
| B/T2 + F.2 签名表 | 跨函数类型地基 | 2 天 | 中 | **最小架构弧②**（主线 301 前） |
| F 核心（检查器最小版：未标注=PyDynamic + 边界清单） | 主线类型洞从"三层补丁"变"签名表加一行" | 2-3 天 | 中（每步门禁） | **最小架构弧③**（主线 301 前） |
...
修复类需要协调。
**2026-09-23 修订（用户裁定"最小架构弧"）**：F 的**最小核心**（签名表 + 检查器最小版）
从"主线后"提前到"主线 301 前"——依据是批次 298-300 的实证：主线剩余障碍（0 成交、
universe 分歧）与已修的三个批次同为"值对、类型丢"类，带检查器上主线，剩余洞的修复
从"三层补丁"降为"签名表加一行"，且 gen.rs 停止喂养。**F.4 全量迁移与 B/T3-T5 装箱
保持主线后不变**——装箱需要 final_value 对齐当正确性标杆，没有标杆不拆发动机。
...
- **第二波（半天~1 天粒度，按依赖排序）**：
  ⑥ ✅ G.2 sanitizer 部分落地（批次 306：ASan 侧工具链就位、MSan 未做、生成码侧未做）
  ...
  ⑨′ 🟡 G.5d 动代码（**批次 319 起诊断侧、批次 320 起可测量侧**）：
    批次 319 = 任务 #33 (a) R7 返回侧告警落地（codegen.rs:3327 调用 / :6960 实现，
  ...
  ⑩ G.3 差分 harness 起步 →
  ⑪ 🟡 性能基线（批次 313：`tools/perf_baseline.py` 落地并采下第一份基线
  ...
  → ⑬ ✅ JIT 静默崩溃根治（批次 315，任务 #26）：⑪ 撞出的"`--emit-llvm` 退出路径
    ...
    (c) 落地形态因此是**填桩而非拒绝**：`jit.rs::trap_unresolved_symbols` 给"被引用 +
...
[backlog.md](backlog.md)**（有界，登记宪法见其文件头）；roadmap.md 回归纯执行日志、
不再新增任务。清理期边界与燃尽序见 backlog.md §0/§3——主线批次 301 冻结至退出条件
触发（S/M 项清零或满 5 个工作日）。
**燃尽序调整（2026-09-23 晚，用户裁定）**：剩余清理时间**全部优先投 B 族解析器截断**
（#36 一族——它是唯一直接影响编译结果正确性的债：还在丢用户代码）；诊断码族按
backlog §3 第 4 条以一行修法收尾即闭；C 族 S 项允许并批一次闭合、跑快速门禁；
...
"F 收敛静态类型（产出边界清单）→ B 落地值模型（按清单装箱、退役探针）→ G.1/G.4
