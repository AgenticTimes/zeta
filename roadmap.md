...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（138 case 全绿）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **138/138**；REasyQuant 语料**完全解析 15/38**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **138/138（0 failed）**；语料「解析通过」**38/38**、「完全解析」**15/38**。

**语料（REasyQuant，38 个文件）** — 度量脚本：`tools/corpus_baseline.py`（解析通过）+ 对 `W1002`「丢了多少行」求和（完全解析口径）

| 指标 | 本阶段起点 | 现在 | 本轮批次 |
|---|---|---|---|
| 完全解析 | 6 → 8 | **15/38** | 12 → 15 |
| 未解析行合计 | 8580 → 6138 | **4846** | 6138 → 4846 |

当前最大拦路：`干积分-量化框架` 376、`小市值排除3bug版` 354、`子账户多策略分仓` 354、`jq_shim` 346、`四大搅屎棍` 343。

**本批次（2026-09-17 追加，commit `9e5cf1ff`）——「parser 误吃前缀」两处 + 一个待修**
...
1. **`//` 整除被当行注释**（本轮新定位，最高性价比）：`x = 7 // 2` 里 `//` 走进 `line_comment`，
   吃掉本行余下文本 ⇒ `int(diff_value // 100) * 100` 变成 `int(diff_value`（括号不闭合）⇒ 该项被丢弃。
   证据：把语料里 `//` 临时换成 `/` 后，`jq_wufu_daily.py` 与 `jq_wufu.py` 未解析行 **72 → 0**，
   `jq_shim.py` **346 → 275**；语料共 19 处 / 9 文件。注意这要新增算子（预处理/parser/MIR/codegen
   四层，且 Python 语义是向下取整而非向零截断），不是纯 parser 修。
2. 语料四个最大拦路（`干积分-量化框架` 376、`小市值排除3bug版` 354、`子账户多策略分仓` 354、
   `四大搅屎棍` 343）**都不含 `//`**，是各自独立的构造，尚未定位——按 §一 的手法逐个走。
3. `slice(...)`：解析通过但链接报 `_slice` 未定义（语料 5 处）→ 需要「slice 句柄作下标」这条路径
4. 默认参数遗留：未标注形参上的字符串/浮点默认值
5. 文末其余未勾选项

> 附记：§一 的「块级 leave-one-out」有一处易踩的假象——**只留块头会制造空块假象**。
