...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（142 case 全绿）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **142/142**；REasyQuant 语料**完全解析 17/38**、未解析行合计 **3372**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **142/142（0 failed）**；语料「解析通过」**38/38**、「完全解析」**17/38**、未解析行合计 **3372**。
...
|---|---|---|---|
| 完全解析 | 6 | **17/38** | 12 → 17 |
| 未解析行合计 | 8580 | **3372** | 6138 → 3372（−2766） |

当前最大拦路（未解析行）：`干积分-量化框架` 376、`jq_wufu_local` 309、`jq_shim` 275、
`十年52倍年化59` 264、`首板低开原版` 230、`高股息价投` 214、`四大搅屎棍` 166、`小市值排除3bug版` 162。

**本批次四（2026-09-17）——逗号下标 `a[i, j]`（commit `8d495a94`）**
...
1. **`for … else:` / `while … else:` 不支持** —— `干积分-量化框架`（376，当前最大文件）的拦路。
   最小复现：`for x in [1,2]:\n    a = x\nelse:\n    a = 3` 让整个顶层项被丢弃。
   严格口径（取同缩进的**最近一条非空行**判断它是不是 `for`/`while`）：全语料只有 **1 处**
   / 1 文件。
   **实现草案**：语义是「循环未 `break` 则执行 else」。需要把本层的 `break` 改写为
   「置标志 + break」（parser 里对循环体语句表做递归重写，注意别碰嵌套循环/函数与
   `if` 内部的 break），再在循环后生成 `if flag == 0 { else_body }`；
   若不想动 break，也可先用「`else` 体在循环后无条件执行（即假设无 break）」——
   **但那会在真有 break 时静默跳过代码，不符合本项目「不静默错值」红线，不建议**。
2. 语料其余拦路：`jq_wufu_local` 309（`_run_local`）、`jq_shim` 275（`get_all_securities`）、
   `十年52倍年化59` 264、`首板低开原版` 230、`高股息价投` 214（余量从 `= [` 开始，
   像某行被从中间切开，值得一看）——尚未定位，按 §一 手法逐个走。
3. **逗号下标已落地为平台桩**（见本批次四）。若要改为响亮失败，删 `py_getitem2` 即可；
   若要真语义，那是 L2 mini-DataFrame 的活。
4. `slice(...)`：解析通过但链接报 `_slice` 未定义（语料 5 处）→ 需要「slice 句柄作下标」这条路径
5. 默认参数遗留：未标注形参上的字符串/浮点默认值；`from X import 常量` 绑定
6. 扁平 python 文件的方言判定、`//=`；文末其余未勾选项
...
> 插 `{`（`opens` 要求下一行缩进更深），于是 harness 自己就先解析失败，看起来像「所有单元都失败」。
>
> 附记四：**数 `else:` 归属不能「往前找到第一个 for/while」**。这样数出 14 处 / 11 文件，
> 严格口径（同缩进最近一条非空行）只有 **1 处**——差的全是「`for` 在前、不相干的 `if/else` 在后」。
> 估算某个构造的价值时，启发式一定要用严格口径重数一遍，否则会把实现优先级排错。
>
