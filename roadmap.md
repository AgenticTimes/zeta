...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（153 case 全绿，逐用例 20s 超时）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **153/153**；REasyQuant 语料**完全解析 36/38**、未解析行合计 **236**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **153/153（0 failed）**；语料「解析通过」**38/38**、「完全解析」**36/38**、未解析行合计 **236**。
