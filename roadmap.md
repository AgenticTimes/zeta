...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（144 case 全绿）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **144/144**；REasyQuant 语料**完全解析 21/38**、未解析行合计 **2641**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **144/144（0 failed）**；语料「解析通过」**38/38**、「完全解析」**21/38**、未解析行合计 **2641**。
...
|---|---|---|---|
| 完全解析 | 6 | **21/38** | 12 → 21 |
| 未解析行合计 | 8580 | **2641** | 6138 → 2641（−3497，−57%） |

当前最大拦路：`jq_wufu_local` 309、`jq_shim` 275、`十年52倍年化59` 264、`首板低开原版` 230、
`高股息价投` 214、`四大搅屎棍`/`小市值排除3bug版`/`子账户多策略分仓` 各 ~162。
...
1. 语料剩余拦路（未解析行 2641）：`jq_wufu_local` 309（`_run_local`）、`jq_shim` 275
   （`get_all_securities`）、`十年52倍年化59` 264、`首板低开原版` 230、`高股息价投` 214
   （余量从 `= [` 开始，像某行被从中间切开）、`四大搅屎棍`/`小市值排除3bug版`/
   `子账户多策略分仓` 各 ~162（都停在 `adjust_stock_num`）。按 §一 手法逐个走。
2. `slice(...)`：解析通过但链接报 `_slice` 未定义（语料 5 处）→ 需要「slice 句柄作下标」这条路径
3. 默认参数遗留：未标注形参上的字符串/浮点默认值；`from X import 常量` 绑定
4. 逗号下标现为**平台桩**（`py_getitem2` 恒等返回句柄）。要改响亮失败：删该 C 函数；
   要真语义：L2 mini-DataFrame。
5. 扁平 python 文件的方言判定、`//=`；泛型函数 `T` 实例化 E2E（`fn id<T>(x: T) -> T` 仍失败，
   但裸 `<` 已不再被误认为泛型实参）；`where T: Ord` 约束检查；文末其余未勾选项
...
> 只堵住了一类：这次 `a.b<10, … c.d>0.3` 的 inner（`10,#…\n c.d`）**本身就是合法类型列表**
> （数字可当类型、点号路径可当类型），于是校验放行。真正的判据不是「内容像不像类型」，
> 而是**这个 `<` 在语法位置上到底是不是泛型实参**——所以最终靠「必须写 `::`」消歧。
> 遇到「启发式校验又漏了」的情况，回到「有没有无歧义的语法标记可用」上想。
>
