...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（139 case 全绿）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **139/139**；REasyQuant 语料**完全解析 17/38**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **139/139（0 failed）**；语料「解析通过」**38/38**、「完全解析」**17/38**。

**语料（REasyQuant，38 个文件）** — 度量脚本：`tools/corpus_baseline.py`（解析通过）+ 对 `W1002`「丢了多少行」求和（完全解析口径）

| 指标 | 本阶段起点 | 现在 | 本轮批次 |
|---|---|---|---|
| 完全解析 | 6 → 8 | **17/38** | 12 → 17 |
| 未解析行合计 | 8580 → 6138 | **4631** | 6138 → 4631 |

当前最大拦路：`干积分-量化框架` 376、`小市值排除3bug版` 354、`子账户多策略分仓` 354、`四大搅屎棍` 343、`首板低开原版` 329、`jq_wufu_local` 309。

**本批次二（2026-09-17 追加）——`//` 整除端到端（commit 见下）**
...
1. 语料六个最大拦路：`干积分-量化框架` 376、`小市值排除3bug版` 354、`子账户多策略分仓` 354、
   `四大搅屎棍` 343、`首板低开原版` 329、`jq_wufu_local` 309。都不含 `//`，是各自独立的构造，
   尚未定位——按 §一 的手法逐个走（`干积分-量化框架` 停在 `def stop_loss`，
   `小市值排除3bug版` 停在 `def get_stock_list`，`jq_wufu_daily` 已全通）。
2. `slice(...)`：解析通过但链接报 `_slice` 未定义（语料 5 处）→ 需要「slice 句柄作下标」这条路径
3. 默认参数遗留：未标注形参上的字符串/浮点默认值
4. 扁平 python 文件的方言判定（见本批次二限界 ①）：无缩进块时 `//` 仍是注释。
   修法需另找方言信号（现仅在 `changed` 后改写），属低优。
5. 文末其余未勾选项
