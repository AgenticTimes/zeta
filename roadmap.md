...
### 同一条搜索表的另一侧：顺带量到一条**新的**静默失效（登记为 #76）
...
⇒ **rc=1**，唯一原因仍是既有判据 `tools/run_all.sh:457`（`py_fail != 0`）：那 2 条还是
...
| **pysrc** | **29 断言 / FAIL 0 / rc=0**（新增，`run_all.sh:382-402`） | — |
| 诊断 official / python_style | 9 文件 15 行 ／ 81 文件 190 行 | 相同 |
...
（`run_all.sh:385` 的"五翼"→"六翼"、批次 342 那三条被挤偏的裸行号引用），行数不变、
...
1. **#76（本批新登记）：相对 `pylib` 基让库面随 CWD 消失，且一声不出。**与 #70 同源、方向
...
- **#76**：本批刚量出来的静默失效，且离刚写好的脚本最近——候选 ③（"换 CWD 读数不变"断言）
  可以在不改编译器的前提下先做，它同时是 ①/② 的验收条件。
- **#70 后半**：判据收紧。需要一次跨位置实验（同一份源码放不同深度各跑一遍），因为盘据 2
  的分布只在本机成立。
