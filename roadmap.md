...
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **137/137**；REasyQuant 语料解析 **38/38**
...
**三套基线**：官方 **194/194**；python_style **137/137（0 failed）**；语料 parse **38/38**。

**语料（REasyQuant，38 个文件）**
...
当前最大拦路：`jq_shim` 445、`jq_wufu_daily` 388、`干积分-量化框架` 376、`jq_wufu` 368。
...
1. `jq_shim.attribute_history` 体内剩余的解析拦路（签名已排除，已能隔离复现）
