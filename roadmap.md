...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（141 case 全绿）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **141/141**；REasyQuant 语料**完全解析 17/38**、未解析行合计 **3889**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **141/141（0 failed）**；语料「解析通过」**38/38**、「完全解析」**17/38**、未解析行合计 **3889**。
...
|---|---|---|---|
| 完全解析 | 6 | **17/38** | 12 → 17 |
| 未解析行合计 | 8580 | **3889** | 6138 → 3889（−2249） |

当前最大拦路（未解析行）：`干积分-量化框架` 376、`jq_wufu_local` 309、`jq_shim` 275、
`小市值排除3bug版` 267、`子账户多策略分仓` 267、`十年52倍年化59` 264、`四大搅屎棍` 258、`国九条中小板微盘` 235。
...
1. **逗号下标 `a[i, j]`（pandas `.iloc[r,c]` / `.loc[r,c]`）——当前最大拦路，约 1667 行**：
   表达式解析器**根本没有**逗号下标这条分支（postfix 的 `[` 只试 `parse_expr` 与 `start:end`）。
   最小复现：`if x[0,0]:` 整个顶层项被丢弃；而 `a = x[0,0]` **能编译但 `a` 拿到的是 `x`**
   （`[0,0]` 作为遗留文本被当成了一个无用的数组字面量语句）—— 又一处静默错值。
   命中：`干积分-量化框架`（`current_data.iloc[0, 0]`）、`小市值排除3bug版`/`子账户多策略分仓`/
   `四大搅屎棍`/`国九条中小板微盘`（`check_limit_up` 同模板）、`十年52倍年化59`（`factor_data.loc[i,factor]`）。
   **需要先做的决策**：① 按 L3「平台 API 是环境不是库」加 no-op shim（`py_getitem2` 返回句柄本身，
   宿主可覆盖）——解析/链接都不破，但本地语义是假值；② 还是只加解析、让它在 gen/link 响亮失败（不静默）。
   选 ① 与现有 `zeta_platform_obj` / 链式方法 identity 兜底一致，选 ② 更符合「不静默错值」。
2. 语料其余拦路：`jq_wufu_local` 309（`_run_local`）、`jq_shim` 275（`get_all_securities`）、
   `十年52倍年化59` 264——尚未定位，按 §一 手法逐个走。
3. `slice(...)`：解析通过但链接报 `_slice` 未定义（语料 5 处）→ 需要「slice 句柄作下标」这条路径
4. 默认参数遗留：未标注形参上的字符串/浮点默认值；`from X import 常量` 绑定
5. 扁平 python 文件的方言判定（见本批次二限界）与 `//=`；文末其余未勾选项

> 附记一：§一 的「块级 leave-one-out」有一处易踩的假象——**只留块头会制造空块假象**。
> 本轮把 `attribute_history` 切成 6 个语句单元时，因为单元边界算错一行，把 dict 字面量的后半
> 当成了独立单元，得到过一整轮错误的「签名/文档字符串有问题」结论。切单元必须**含完整子块**，
> 且先用单条语句逐个隔离过一遍再组合。
>
> 附记三：**隔离 harness 里 stub 不能写成单行 def**。`def f(a): return a` 在 python 风格下不会被
> 插 `{`（`opens` 要求下一行缩进更深），于是 harness 自己就先解析失败，把余量定在 harness 第一行，
> 看起来像「所有单元都失败」。stub 一律写多行。
>
