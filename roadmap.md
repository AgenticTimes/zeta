...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（148 case 全绿）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **148/148**；REasyQuant 语料**完全解析 32/38**、未解析行合计 **881**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **148/148（0 failed）**；语料「解析通过」**38/38**、「完全解析」**32/38**、未解析行合计 **881**。
...
|---|---|---|---|
| 完全解析 | 6 | **32/38** | 12 → 32 |
| 未解析行合计 | 8580 | **881** | 6138 → 881（−5257，−86%） |

剩余拦路：`jq_wufu_local` 309（`_run_local`）、`jq_shim` 275（`get_all_securities`）、
...
1. 语料剩余拦路（未解析行 881）：`jq_wufu_local` 309（`_run_local`）、`jq_shim` 275
