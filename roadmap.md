...
接线 6 处：usage `tools/run_all.sh:3`、`SKIP_IGNORE=0` `:25`、`--skip-ignore` `:42`、步骤块 `:431-456`、JSON `ignore_rules` 键 `:485-486`、判据 `:535`。
...
`python3 tools/check_abi_anchors.py`（不经管道）⇒ 插入第 13 步后**漂移 3**：`tools/run_all.sh:97`、`:192`、`:483`。`--rebind` ⇒ 3 条全部"逐字相同 + 全文件唯一命中"，判为搬家 `:99` / `:194` / `:513`，改写 `docs/ABI.md` 3 行、`tools/baselines/abi_anchors.tsv` 3 行。复跑 rc=0：**243 个可解析 / 0 定位失败，漂移 0 / 新 0 / 消失 0，待归属 93 条 / 84 种，声明仓外 14 条** —— 与批次 345 完全一致。

`roadmap.md` 正文里被本批换号的 4 种裸引用（核对器看不见，#52 欠的债又厚一层）。历史读数**不回改**，此处只列旧号 → 新号：

| 引用处 | 旧号 | 现号 | 是哪一行 |
|---|---|---|---|
| `:12625` / `:12794` / `:12979` | `run_all.sh:484` | `:514` | `py_fail != 0 ⇒ rc=1` 那条唯一判红处 |
| `:12645` | `run_all.sh:384-404` | `:386-406` | 第 11 步（pysrc）块 |
| `:12652` | `run_all.sh:387` | `:389` | "五翼→六翼"那句注释 |
| `:12813` | `run_all.sh:406-426` | `:408-428` | 第 12 步（cli_semantics）块 |
...
### 门禁读数（十三步，`./tools/run_all.sh > /tmp/b346_gate.log 2>&1`，rc 从 `/tmp/b346_gate.rc` 直读；ts `2026-09-23T00:30:33Z`）

⇒ **rc=1**，唯一原因仍是既有判据 `tools/run_all.sh:514`（`py_fail != 0`，那 2 条还是 `t231_dict_set_cast_fromkeys` / `t233_listcomp_condition_capture`）。
...
| knob | 23 断言 / FAIL 0 | 同 | 不变 |
| swallow | 4 / 0 | 同 | 不变 |
| import_form | 22 / 0 | 同 | 不变 |
| empty_stmt | 68 / 0 | 同 | 不变 |
| clean_checkout | rc=0（0s，`rev=95c5c8c0`） | rc=0（1s，`rev=8447af4b`） | 唯一差异是 HEAD 换成本批父提交；`rev` 本来就是每次跟着 HEAD 走 |
| pysrc | 29 / 0 / rc=0 | 同 | 不变 |
| cli_semantics | 18 / 0 / rc=0 | 同 | 不变 |
| **ignore_rules** | **19 / FAIL 0 / rc=0** | —（本批新增） | 第 13 步上线 |

逐步文本与 345 的日志做 `diff`：11 行里 10 行逐字相同，唯一不同就是上面 `clean_checkout` 那行的 `rev`/`secs`。门禁侧对"改 ignore 规则"零敏感，这正是本批想要的判读 —— 规则表的可信度从此由第 13 步单独担保，不再靠"别的步骤还绿着"这种旁证。
...
5. **新出现的用户文件 `zeta_src/qwenadvice.md`（20,575 B）只报告、不提交、不删除。**
