...
> 测试资产：官方单测 **`tests/unit-tests/`（194 文件，进 git 的正本）**；回归套件 `/tmp/bench`；**Python 风格套件 `tests/python_style/`（152 case 全绿，逐用例 20s 超时）**
> 当前通过率（2026-09-17 实测，validate.md §3 口径）：官方 **194/194**（运行退出码与基线零差异）；python_style **152/152**；REasyQuant 语料**完全解析 35/38**、未解析行合计 **382**（`ZETA_STRICT_PARSE` 口径，退出码见 §二）
...
**三套基线**：官方 **194/194**；python_style **152/152（0 failed）**；语料「解析通过」**38/38**、「完全解析」**35/38**、未解析行合计 **382**。
...
|---|---|---|---|
| 完全解析 | 6 | **35/38** | 12 → 35 |
| 未解析行合计 | 8580 | **382** | 6138 → 382（−5756，−94%） |

剩余拦路（3 个文件）：`jq_wufu_local` 159（`_run_nautilus`）、`指数ETF动量轮动` 146（`initialize`）、
`ETF动量EPO` 77（`epo(…)`，里面有 **`@` 矩阵乘**，本编译器不支持该算子）。
