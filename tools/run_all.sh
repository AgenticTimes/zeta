...
# 本步骤钉六翼（344 三翼 + 348 两翼 + 349 一翼）：裸跑必须执行（探测器阳性对照）、五个只读入口必须不执行且 rc=0、
...
# cli_semantics: 判据在 tools/cli_semantics_check.sh 内部（六翼），这里只认退出码。翼 C 的 --repl 判据自带 `head -c` 保险丝，跑飞也不会把门禁挂住。
