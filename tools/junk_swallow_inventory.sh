...
# 曾经在册的两条真阳性已各自补上规则收掉：`import std::memory;`（绑到 `std`，
# `memory` 作为裸名语句消失）由批次 338 收进 `import` 的 `::` 翼，
# `static mut counter: i64 = 0`（`static` 消失，剩下 `mut …` 当场中止 ⇒ W1002 截断）
# 由批次 384 补 `parse_static` 收掉。全语料实测：合计 0 处 / 549 文件
# （批 383 收掉闭括号噪声后剩的 1 处真阳性就是 `static`，本批归零）。
# 留着这两条是因为它们标记了本判据的适用范围——`import` 那条的回归见
# tools/import_form_inventory.sh。吞词发生在"解析器没有规则的前导词"上，
# 规则补齐一处就少一处。
# W1004（批次 337）就在吞掉的那一刻说话。它的判据有排除项（隐式返回长得一样），
# 而"有排除项的判据"最容易在两翼上各翻一次：永远不打（判据写死了）和到处乱打
# （把隐式返回当 bug）。每种形状各做成夹具，另加全语料计数当读数。
# 排除项的现值域（批次 383 量出来的）：函数尾隐式返回、`} else {` 那型、以及
# 下一行以 `}`/`)`/`]` 开头的"括号内尾值"（闭包尾巴 `});`、单行 match `}, _ => 0 }`）。
#
# 用法：
...
print(f())
EOF
# 夹具 4：应当**哑**——尾巴表达式的下一行以闭括号开头（批次 383 的排除项）。
# 这两族在改前各自实拍：`});` 16 声、`}, _ => 0 }` 3 声（同一份夹具，构建只差这一条
# 判据），程序输出两版都是 8 ⇒ 排除的是噪声，不是值。
cat > "$TMP/closer-tail.z" <<'EOF'
fn g2(f: i64) -> i64 {
    return 0
}
fn g(n: i64) -> i64 {
...
want "尾表达式的下一行以闭括号开头 ⇒ 不响" 0 "$(hits "$TMP/closer-tail.z")"
got=$("$ZETAC" "$TMP/closer-tail.z" 2>/dev/null | grep -v 'clang:' | head -1)
want "同上：臂体尾值与副作用仍进结果（闭括号排除没吃掉值）" 8 "$got"
