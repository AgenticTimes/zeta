...
#   负控制（D）  —— 接受空项不等于放宽吞词判据：`!!!` 仍然截断，`q;` 仍然响 W1004
#                  （批次 337 的判据与本批共用语句兜底，最容易顺手关掉）。
...
# 期望值来自实测。这条曾是 D 翼的负控制成员（`static` 被吞 ⇒ W1002+W1004）；
# 批次 384 给它加了 `parse_static`，同一个夹具实测"无诊断"，于是它改成一条回归锁：
# 顶层 `static mut` 已被接住，不许再退回去响。负控制仍由上面两条（`!!!`、`q;`）承担。
want "顶层 static mut 已被接住，一声不出（批 384 前响 W1002+W1004）" "" "$(diag "$TMP/n_static.z")"
