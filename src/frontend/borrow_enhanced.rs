...
impl EnhancedBorrowChecker {
    ...
    pub fn move_variable(&mut self, name: &str) -> Result<(), String> {
        ...
        if !info.can_move() {
            return Err(format!(
                "Cannot move '{}': still borrowed or not owned",
                name
            ));
        }
        ...
        match node {
            AstNode::Var(name) => {
                if !self.can_use(name) {
                    crate::diag_error!("E4001", "Borrow error: Cannot use variable '{}'", name);
                    return false;
                }
                true
            }
            ...
            AstNode::Assign(lhs, rhs) => {
                ...
                match **lhs {
                    ...
                    _ => {
                        crate::diag_error!("E4003", "Borrow error: Invalid assignment target");
                        false
                    }
                }
            }
