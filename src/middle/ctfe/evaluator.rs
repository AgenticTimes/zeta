...
use super::visitor::{AstTransformer, AstVisitor};
...
impl ConstEvaluator {
        ...
        match node {
            AstNode::ConstDef {
                ...
                match self.eval_const_expr(value) {
                    ...
                    Ok(ConstValue::Bool(val)) => Ok(AstNode::ConstDef {
                        name: name.clone(),
                        ty: ty.clone(),
            ...
            | AstNode::Use { .. }
            | AstNode::TypeAlias { .. }
            | AstNode::AssociatedType { .. } => self.transform_expr(node),
    ...
    fn transform_expr(&mut self, node: &AstNode) -> CtfeResult<AstNode> {
        ...
        let transformed = match node {
                ...
                match &transformed_expr {
                    AstNode::Lit(_) | AstNode::Bool(_) => {
                        match self.eval_unary_op(op, &transformed_expr) {
                            Ok(ConstValue::Int(result)) => AstNode::Lit(result),
                            Ok(ConstValue::Bool(result)) => AstNode::Bool(result),
                            Ok(_) => AstNode::UnaryOp {
                                op: op.clone(),
                                expr: Box::new(transformed_expr),
                            },
                            Err(e) => AstNode::UnaryOp {
                                op: op.clone(),
                                expr: Box::new(transformed_expr),
                            },
                        }
            ...
            AstNode::Call {
                ...
                let transformed_args: Result<Vec<_>, _> =
            ...
            AstNode::If { cond, then, else_ } => {
                ...
                let transformed_then: Result<Vec<_>, _> =
                    then.iter().map(|stmt| self.transform_expr(stmt)).collect();
                let transformed_then = transformed_then?;
                let transformed_else: Result<Vec<_>, _> =
                    else_.iter().map(|stmt| self.transform_expr(stmt)).collect();
    ...
    pub fn eval_const_expr(&mut self, expr: &AstNode) -> CtfeResult<ConstValue> {
        match expr {
            ...
            // Binary operations
            AstNode::BinaryOp { op, left, right } => self.eval_binary_op(op, left, right),

            // Unary operations
            AstNode::UnaryOp { op, expr } => self.eval_unary_op(op, expr),

            // Variables
            AstNode::Var(name) => self.eval_variable(name),
            ...
            // Arrays
            AstNode::ArrayLit(elements) => self.eval_array_literal(elements),

            // Expression statements — unwrap and evaluate inner expression
            AstNode::ExprStmt { expr } => self.eval_const_expr(expr),

            // If expressions
            AstNode::If { cond, then, else_ } => self.eval_if_expr_with_else(cond, then, else_),

            // Blocks
            AstNode::Block { body } => self.eval_block(body),
            ...
            AstNode::Assign(target, value) => self.eval_assignment(target, value),
            ...
            _ => Err(CtfeError::UnsupportedExpression(format!("{:?}", expr))),
        }
    }
    ...
    fn eval_user_function_call(&mut self, name: &str, args: &[AstNode]) -> CtfeResult<ConstValue> {
        ...
        if self.recursion_depth >= MAX_RECURSION_DEPTH {
            return Err(CtfeError::FunctionCallFailed(format!(
                "recursion depth limit ({}) exceeded for '{}'",
        ...
        match func {
            AstNode::FuncDef {
                ...
                if !const_ && !comptime_ {
                    return Err(CtfeError::FunctionCallFailed(format!(
                        "function '{}' is not const or comptime",
                        name
                    )));
                }
                ...
                for (i, (param_name, _param_type)) in params.iter().enumerate() {
                    self.context
                        .define_variable(param_name.clone(), arg_values[i].clone())?;
                }
            ...
            _ => Err(CtfeError::FunctionCallFailed(format!(
                "expected function definition for '{}'",
                name
            ))),
        }
    }
    ...
    fn eval_array_literal(&mut self, elements: &[AstNode]) -> CtfeResult<ConstValue> {
        ...
        if const_elements
            .iter()
            .all(|e| matches!(e, ConstValue::Int(_)))
        {
    ...
    fn eval_assignment(&mut self, target: &AstNode, value: &AstNode) -> CtfeResult<ConstValue> {
        let val = self.eval_const_expr(value)?;
        ...
        match target {
            ...
            AstNode::Subscript { base, index } => {
                let base_var = match base.as_ref() {
                    ...
                    other => {
                        return Err(CtfeError::UnsupportedExpression(format!(
                            "complex array subscript target not supported: {:?}",
                            other
                        )));
                    }
                };
                ...
                self.context
                    .assign_array_element(&base_var, idx as usize, val.clone())?;
                Ok(val)
            }
            AstNode::Var(name) => {
    ...
    fn eval_for_loop(
        ...
        let (start_expr, end_expr, inclusive) = match expr {
            ...
            AstNode::BinaryOp {
                op, left, right, ..
            } if op == ".." => (left.as_ref(), right.as_ref(), false),
            _ => {
                return Err(CtfeError::UnsupportedExpression(format!(
                    "for loop range must be start..end or start..=end, got: {:?}",
                    expr
                )));
            }
        };
        ...
        for i in start..range_end {
            ...
            // Bind loop variable
            self.context
                .define_variable(var_name.clone(), ConstValue::Int(i))?;
