            ...
            AstNode::Call { method, args, .. } => {
                ...
                if let Some(sig) = self.get_func_signature(method) {
                    ...
                    if !method.contains("::") && !method.contains('<') {
                        let diag = crate::error_codes::diagnostic_from_code(
                            "E2006",
                            format!("Unknown function {}", method),
                            None,
                        );
                        crate::diagnostics::emit(diag);
                    }
                            ...
                            (lty.as_vector(), rty.as_vector())
                        {
                            if l_inner != r_inner {
                                let diag = crate::error_codes::diagnostic_from_code(
                                    "E2024",
                                    format!(
                                        "SIMD vector element type mismatch in '{}': {} vs {}",
                                        op,
                                ...
                                let diag = crate::error_codes::diagnostic_from_code(
                                    "E2024",
                                    format!(
                                        "SIMD vector size mismatch in '{}': {} vs {}",
                                        op, l_size, r_size
                                    ),
                                    None,
                                );
    ...
    fn string_to_type(&self, s: &str) -> Type {
        ...
        match self.parse_type_string(s) {
            ...
            Err(err) => {
                let diag = crate::error_codes::diagnostic_from_code(
                    "E2005",
                    format!("Failed to parse type '{}': {}", s, err),
                    None,
                );
                crate::diagnostics::emit(diag);
            ...
            match verify_identities(ast) {
                Ok(warnings) => {
                    ...
                    for warning in warnings {
                        let diag = crate::diagnostics::Diagnostic::warning(
                            "W0003",
                            format!("Identity warning: {}", warning),
                        );
                        crate::diagnostics::emit(diag);
                    }
                }
